...
use nia_llvm::{basic_block::BasicBlock, values::BasicValueEnum};
use nia_span::Span;
...
impl<'m, 'ctx, 'a> FunctionCodegen<'m, 'ctx, 'a> {
    ...
    pub(super) fn is_void_expr(&self, expr: &TypedExpr) -> bool {
    ...
    pub(super) fn expr_requires_void_emit(&self, expr: &TypedExpr) -> bool {
        matches!(
            expr.kind,
