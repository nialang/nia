...
pub use allocation::{
    AllocationLiveSnapshot, AllocationLiveWindowMeasurement, CountingAllocator,
    allocation_live_snapshot, measure_allocation_live_window, register_allocation_instrumentation,
};
use allocation::{AllocationMeasurement, finish_allocation_tracking, start_allocation_tracking};
#[cfg(test)]
use allocation::{current_live_bytes, record_allocation, record_deallocation, record_reallocation};
use report::{TimingCounter, TimingReport, TimingReportBuilder, print_report};
#[cfg(test)]
use report::{TimingReportEntry, format_event, format_json_report, format_report_entry};
...
fn print_event(event: &TimingEvent) {
    eprintln!("{}", report::format_event(event));
}
...
mod tests {
    ...
    fn allocation_tracking_records_only_the_active_interval() {
        ...
        let baseline = current_live_bytes();
