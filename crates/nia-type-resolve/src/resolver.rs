...
#[derive(Clone, Copy)]
pub(super) struct TypeResolveOptions<'a> {
    pub(super) symbols: Option<&'a dyn SymbolText>,
    pub(super) mode: TypeResolveMode,
    pub(super) node_store: &'a NodeStore,
}
...
pub(super) enum TypeResolveMode {
    All,
    Declarations,
}
...
impl<'a> TypeResolver<'a> {
            ...
            ResolvedNamespace::Module(module_id) => {
                if let Some(surfaces) = self.public_surfaces {
                    if let Some(child_module) =
                        surfaces.public_module(module_id, type_segment_name(segment)?)
                    {
                        return Some(ResolvedNamespace::Module(child_module));
                    }
                    if let Some(item) = surfaces.public_type(module_id, type_segment_name(segment)?)
                    {
                        return Some(ResolvedNamespace::Type(GlobalDefId {
    ...
    fn try_resolve_module_type(
        ...
        if let Some(surfaces) = self.public_surfaces
            && let Some(item) = surfaces.public_type(module_id, type_segment_name(segment)?)
        {
    ...
    fn resolve_module_type(
        ...
        if let Some(surfaces) = self.public_surfaces
            && let Some(item) = surfaces.public_type(module_id, &name)
        {
            let global = GlobalDefId {
                module_id: item.target_module,
                def_id: item.target_def_id,
    ...
    fn with_generics(&mut self, generics: &[GenericParam], f: impl FnOnce(&mut Self)) {
        ...
        f(self);
        // These stacks are dynamic resolver context, not accumulated facts.
        // Always restore them after visiting a nested item so its generic,
        // `Self`, or associated-type names cannot leak into later siblings.
