...
pub(super) fn provide_extension_signature_inputs(
    ...
    let modules = db.query_many(
        db.query(ExtensionProviderModuleIdsQuery)
            .into_iter()
            .map(ExtensionSignatureModuleInputQuery),
    );
...
fn executable_checked_modules_inner(db: &QueryDb<CompilerContext>) -> Vec<CheckedModule> {
    ...
    let function_signature = |def_id: GlobalDefId| {
        ...
        let signature = body_local_item_signatures(db, def_id.module_id, &lowered)
            .functions
            .get(&def_id.def_id)
            .cloned()
    ...
    let reachability = loop {
        ...
        while let Some(module_id) = stale.pop_front() {
            ...
            for next_module_id in parse_ok.iter().copied() {
                ...
                let is_stale =
                    executable_module_is_stale(next_module_id, &reachability, &checked_by_id);
