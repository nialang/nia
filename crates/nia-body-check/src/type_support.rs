...
impl<'a> BodyChecker<'a> {
    ...
    pub(crate) fn clone_for_type_compare(&self) -> BodyChecker<'a> {
        BodyChecker {
            ...
            field_default_sources: self.field_default_sources.clone(),
            field_default_templates: self.field_default_templates.clone(),
            active_field_default_templates: HashSet::new(),
            current_field_default_owner: self.current_field_default_owner,
            next_instantiated_local_id: self.next_instantiated_local_id,
    ...
    pub(crate) fn string_literal_array_type(
        ...
        let len = string_literal_char_len(literal)
            .map(|len| {
                u64::try_from(len)
                    .map(ArrayLenTy::ConstValue)
                    .unwrap_or(ArrayLenTy::Infer)
            })
            .unwrap_or(ArrayLenTy::ConstValue(0));
    ...
    pub(crate) fn byte_string_literal_array_type(
        ...
        let len = byte_string_literal_len(literal)
            .map(|len| {
                u64::try_from(len)
                    .map(ArrayLenTy::ConstValue)
                    .unwrap_or(ArrayLenTy::Infer)
