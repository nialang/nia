...
use crate::used_paths::{ExplicitUsingImport, ModuleDeclarations};
...
use public_surface_using::{
    public_surface_fact_symbols, read_module_using, validate_module_using, write_module_using,
};
use used_module_path::{
    collect_used_module_path_symbols, read_used_module_path, write_used_module_path,
};
...
fn validate_public_surface_facts(
    ...
    // Cached facts are trusted only after their canonical ordering, reference
    // closure, namespace kinds, and source-relative spans agree as one unit.
    facts
        .defs
        .windows(2)
