...
fn compiler_cases_match_snapshots() {
    ...
    run_persistent_check_suite(&root);
...
fn run_persistent_check_suite(root: &Path) {
    let suite_root = root
        .parent()
    ...
    for case_root in cases {
        ...
        run_persistent_check_case(&case_root);
    }
}

fn run_persistent_check_case(case_root: &Path) {
    let case = load_persistent_check_case(case_root);
    let workspace = super::common::temp_dir("persistent-check-case");
    copy_case_tree(case_root, &workspace);
    let source = workspace.join(&case.source);
    assert!(
        source.is_file(),
...
fn load_persistent_check_case(case_root: &Path) -> PersistentCheckCase {
    let mut manifest = CaseManifest::load(case_root);
    let manifest_path = manifest.path.clone();
