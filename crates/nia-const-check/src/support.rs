...
pub(crate) fn validate_assignment_shape(
    ...
    match (value, previous) {
        (ConstValue::Array(values), ConstValue::Array(previous_values)) => {
            if values.len() != previous_values.len() {
                diagnostics.push(Diagnostic::user_error_at(
                    codes::CONST,
                    span,
                    format!(
                        "const array length {} does not match expected length {}",
                        values.len(),
                        previous_values.len()
                    ),
                ));
        ...
        (ConstValue::Optional(Some(value)), ConstValue::Optional(Some(previous))) => {
            validate_assignment_shape(diagnostics, span, value, previous, symbol_name);
        }
        (ConstValue::ErrorUnion(Ok(value)), ConstValue::ErrorUnion(Ok(previous)))
        | (ConstValue::ErrorUnion(Err(value)), ConstValue::ErrorUnion(Err(previous))) => {
            validate_assignment_shape(diagnostics, span, value, previous, symbol_name);
        }
        _ => {}
    }
...
pub(crate) fn const_string_to_char_array(value: &str) -> Vec<ConstValue> {
    value
        .chars()
...
pub(crate) fn cast_const_integer(
    value: IntConst,
    ty: PrimitiveTy,
    pointer_width: u32,
) -> Option<IntConst> {
