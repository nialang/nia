...
use super::{
    ...
    compiler_check::{
        COMPILER_CHECK_COMPILATION_MODE_DOMAIN, COMPILER_CHECK_MODULE_DOMAIN,
        COMPILER_CHECK_OPTIMIZATION_DOMAIN, COMPILER_CHECK_PACKAGE_ROOTS_DOMAIN,
        COMPILER_CHECK_PROFILE_DOMAIN, COMPILER_CHECK_RUNTIME_DOMAIN, COMPILER_CHECK_TARGET_DOMAIN,
        CompilerCheckCacheIdentity, CompilerCheckCacheIdentityInput, FingerprintComponents,
...
impl CompilerEmitCacheIdentity {
    pub(crate) fn new(input: CompilerEmitCacheIdentityInput<'_>) -> Option<Self> {
        ...
        Some(Self {
            ...
            compilation_mode: compiler.compilation_mode,
...
fn write_fingerprint_set(encoded: &mut Vec<u8>, fingerprints: EmitFingerprintSet) {
    for fingerprint in [
        ...
        fingerprints.components.compiler.profile,
        fingerprints.components.compiler.compilation_mode,
...
mod tests {
    ...
    fn identity() -> CompilerEmitCacheIdentity {
        ...
        let compiler = FingerprintComponents {
            ...
            target: bytes_fingerprint(COMPILER_CHECK_TARGET_DOMAIN, &target),
            profile: integer_fingerprint(COMPILER_CHECK_PROFILE_DOMAIN, 0),
            compilation_mode: integer_fingerprint(COMPILER_CHECK_COMPILATION_MODE_DOMAIN, 0),
            optimization: integer_fingerprint(
                COMPILER_CHECK_OPTIMIZATION_DOMAIN,
                u64::from(optimization),
            ),
    ...
    fn compiler_emit_invalidation_is_component_exact() {
        ...
        for (changed, expected) in [
            ...
            (
                EmitFingerprintComponents {
                    compiler: FingerprintComponents {
                        compilation_mode: fingerprint(20),
                        ..baseline.compiler
                    },
                    ..baseline
                },
