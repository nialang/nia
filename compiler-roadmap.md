...
| C 后 ID/arena 专项 | 约 76% | ID-0 query dep-node arena 与 ID-1 source/syntax identity 已完成；ID-2 已建立 stable/local module 映射、删除 full-node query，并让 loader/compiler/checked/codegen 共享 immutable graph snapshot；production 已收敛到显式 allocator，单模块与大部分 compiler-query fixture 已迁移到真实 graph handle。 |
