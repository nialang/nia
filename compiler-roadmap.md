...
| C query value/storage | 约 65% | cache-owned `get/try_get/get_many` 与无 `Value: Clone` 已建立；loader、compiler semantic/body/check/fact batch/const facts 与 executable function lowering 已共享，compiler-query 的 legacy `query_many` 已清零，剩余 owned `query`、backend/program aggregate 与 registry 尚未完成。 |
