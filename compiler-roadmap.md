...
| F IR ownership / item 粒度 | 约 30% | interner 已从 body/backend 产品移除；lowered Function IR 已进入query-product-owned immutable typed store，backend 全程序索引只借用store payload，并已删除materialization/instance discovery前的重复深拷贝。per-function query、checked-body store、owned extraction、及时释放、per-CGU lowering和peak-live-bytes验证尚未建立。 |
