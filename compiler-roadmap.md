...
| C query value/storage | 约 25% | `QueryKey::Value` 的通用 `Clone` 约束已删除，cache-owned `get/try_get` 已建立，loader 与 compiler resolution/type-resolution/lowering 产品已共享到 aggregate；其余 owned `query/query_many`、normalization/check 产品与 declarative registry 尚未完成。 |
