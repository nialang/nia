...
| F IR ownership / item 粒度 | 约 48% | interner 已从body/backend产品移除；checked body与static init payload由aggregate/item query共享，lowered Function IR已成为per-function query product；backend只借用query-owned function/static IR，且无consumer的过渡store/ID、batch API与`BodyIr` backend入口已删除。checked-body/static-init item producer、owned extraction、及时释放、per-CGU lowering和peak-live-bytes验证尚未建立。 |
