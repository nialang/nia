...
| 阶段 | 当前估算 | 判断 |
|---|---:|---|
| A 基线与防回归 | 约 85% | 六 workload、machine-readable 指标、allocator traffic/current/peak-live、query/LLVM counters 与同机 guard 已完成；backend module fan-out已有三点live/peak观测，baseline comparator同时守卫allocated与peak-live bytes。可运行 LLVM suite 的 CI 和 main-branch trend storage 未完成，但不阻塞当前进程内 identity/query/fact graph 临界路径。 |
