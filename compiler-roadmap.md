...
| D 统一依赖图 | 约 80% | loader/provider update 已收敛为 revisioned typed worklist，compiler update 已由 cache-owned provider/body worklist 与 executable epoch 驱动；通用 invalidation 会保留稳定 transitive dependents 做 demand-time validation，graph relation、loaded/parse/semantic/program-signature/extension membership、source version、public/using relation与 provider eligibility 已接入稳定事实层，但 source existence 和 loader/compiler 单一 session DB 尚未完成。 |
...
综合判断：**整份路线图按剩余工程复杂度加权约完成 51%，合理区间为 49%–53%；A–E 的 P0 基础约完成 73%–78%。** 已完成的是最先阻塞后续工作的 type identity、query storage、loader-owned provider update、typed compiler worklist、通用 derived red-green 内核路径及稳定 compiler module graph/membership facts；跨数据库统一 fact graph、executor、item/CGU 粒度和持久增量仍是独立的大型工程，不能按提交数量线性外推后半程。

ID/arena 专项已关闭，当前临界路径继续推进 Phase D：driver 重复协议和 compiler fact-session update mutation 已删除，provider/body delta 与 executable reset 都由 typed query input 表达；stable derived slot 已锁定 validation/invalidation 并发状态、连续 revision 与 `get_many` 依赖记录，graph relation、loaded membership 与 public/using relation query 都缓存 stable source identity，source version 以完整 local handle/revision 防止旧身份误转 green。下一步把 source existence 纳入正式 query fact，并删除 loader/compiler 之间剩余的重复 graph truth source；Phase A 剩余 CI/trend storage 在托管环境和基线存储策略明确前不抢占该路径。
