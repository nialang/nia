...
| F IR ownership / item 粒度 | 约 68% | checked body、static init与lowered Function IR均已成为item-owned product；backend只借用query-owned function/static IR，无consumer的过渡store/ID、batch API与`BodyIr` backend入口已删除。monomorphization/backend aggregate也已成为production tracked query owner，但global discovery、per-item/per-CGU materialization、owned extraction、及时释放和peak-live-bytes验证尚未建立。 |
