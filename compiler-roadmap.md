...
| D 统一依赖图 | 约 93% | loader/provider update 已收敛为 append-only revision events 与 revision-keyed graph query，compiler update 已由 cache-owned provider/body worklist 与 executable epoch 驱动；loader/compiler typed DB 共享同一 `QuerySession`，compiler graph/membership/diagnostics roots 已直接依赖 loader query，Driver 的 `LoadedProgram → CompileRequest` 与 pending program/revision DTO 已删除。module field snapshot、provider lifecycle 和 reachability scheduler 尚未完全进入统一图。 |
...
ID/arena 专项已关闭，当前临界路径继续推进 Phase D：driver 重复摘要和 compiler fact-session update mutation 已删除，provider/body delta 与 executable reset 都由 typed query input 表达；loader graph 是 provider revision-keyed query product，loader/compiler typed DB 共享同一 QuerySession dependency arena，graph/membership/diagnostics 三类 production compiler roots 已真实调用 loader query 并记录跨 DB edge，Driver 不再物化或传递 `LoadedProgram`。下一步把 source/module fields、target/runtime/symbol/provider revision roots逐项迁到同一 fact provider，并删除 `CompilerInputs::new` 对 aggregate loaded snapshot 的依赖，再收敛 provider lifecycle/reachability worklist；不能把“首批 root 已迁移”虚报为全部 loader facts 已统一。Phase A 剩余 CI/trend storage 在托管环境和基线存储策略明确前不抢占该路径。
