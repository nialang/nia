...
| G CGU / 异步 codegen / work products | 约 95% | deterministic bounded source bucket、跨session stable CGU key、versioned CGU/link-result component fingerprint、owned readonly whole-program context、统一session codegen queue、per-unit LLVM ownership/memory backpressure、持久native object/link-result cache、typed incremental link inputs及两层精确invalidation reason已完成；代表性large workload确认1→4 unit有吞吐收益，但task数超过memory capacity会抬高worker allocator高水位，frontend/LLVM尚未overlap。 |
