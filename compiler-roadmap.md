...
| I 错误、诊断与工程重组 | 约 48% | query runtime 已建立显式 provider failure channel，cycle、invalid input 与 speculative dependency cleanup 不再要求 runtime downcast panic payload；loader、compiler invalid-input origin、frontend type/value基础图、标准const lowering、flow/ABI check、extension discovery/facts/index/visibility及item value-ref edge链均已迁移，发散型 invalid-input panic helper已删除。裸 `execute`/`get` 迁移 adapter、其余 compiler const/layout/static/body/codegen与extension validation provider graph、diagnostic store、data-driven integration harness、外层 session weight替代方案和 crate/巨型文件重组仍待推进。 |
