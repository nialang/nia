...
| C/D 后 revision retirement | 约 65% | 通用 quiescent retirement、五类旧 `SourceVersion` query 回收和 provider graph lineage sealing 均已建立；source edit与provider add/reset各100轮后live slots/transition有界，slot ID不复用，外部旧`Arc`仍独立安全。剩余共享`NodeStore` revision locator owner尚需收口。 |
