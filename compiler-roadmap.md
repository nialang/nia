...
| F IR ownership / item 粒度 | 约 75% | checked body、static init与lowered Function IR均已成为item-owned product；backend只借用query-owned function/static IR，无consumer的过渡store/ID、batch API与`BodyIr` backend入口已删除。monomorphization/backend aggregate已有production query owner，per-module frontend source-item与function-instance plan也已成为tracked semantic product；但body/vtable诱发instance、global-instance/vtable closure、per-item/per-CGU materialization、owned extraction、及时释放和peak-live-bytes验证尚未建立。 |
