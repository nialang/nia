...
| G CGU / 异步 codegen / work products | 100% | deterministic bounded source bucket、跨session stable CGU key、versioned CGU/link-result component fingerprint、owned readonly whole-program context、资源有界LLVM worker lanes、持久native object/link-result cache、typed incremental link inputs及两层精确invalidation reason已完成；ready partition以bounded priority task接入同一persistent executor并与frontend finalization真实overlap。clean revision的3× fixed workload已锁定4 units/2 lanes/4 ready submissions及CPU/RSS/finalization live window，架构与资源acceptance关闭；跨main长期trend storage唯一归Phase A。 |
| H 持久 frontend incremental | 100% | stable module/def identity、source/syntax/item-signature与配置fingerprint、v3 symbol-aware dependency/provider/facade/public-surface产品、canonical signature/extension产品、executable value-ref edges与loader-owned settled provider-demand plan均完成corruption retirement和verification。正式check report与typed analysis拆分后，exact-input clean-check certificate使代表workload第二次运行从9430降至770次execution、semantic/body链全部为0、wall约1.69s降至19ms；单文件编辑继续由既有细粒度产品精确失效。非空diagnostic bundle持久化明确归Phase I，不再阻塞H的clean validation acceptance。 |
| I 错误、诊断与工程重组 | 约 75% | panic-based query failure及访问API双轨已100%删除；stable single-source/program diagnostic bundle、non-clean check certificate及覆盖check/multi-module/incremental/persistent/codegen/LLVM/object/link/execute的data-driven suite已完成，普通driver unit tests不再获取全局compiler permit，首个巨型test harness、compiler query registry、context数据、五批query support/fixture及frontend/provider/persistent/body四批实际incremental cases也已按职责拆分，crate consumer matrix已有可重复工具。尚未完成的是session-owned `DiagnosticId`的其余高扇出迁移、更宽阈值crate边界复核及证据成立时的实际合并、其余巨型production/test文件拆分，以及剩余临时调试入口清理。此前约98%的过程估计只衡量了panic-flow主线，未逐项验收Phase I五项，现已纠正。 |

综合判断：**核心编译器架构主线约完成99%，但按roadmap全部显式Acceptance加权约完成95%，合理区间为94%–96%。** A–H 的P0/P1 ownership、runtime、codegen与persistent frontend基础已完成；未完成量主要不是新的编译语义，而是Phase I的session diagnostic ownership、crate/file重组、data-driven测试体系与临时资源入口退休，以及Phase A唯一负责的托管CI/main-branch trend storage。已完成的是 type identity、query storage、统一 revisioned fact graph、session 内持久 query调度、进程级CPU/LLVM memory协调、item-owned backend IR、deterministic bounded source partition、stable CGU/link-result component fingerprint、真实per-unit LLVM task queue、持久object/link-result cache、typed incremental link inputs与两层精确invalidation reason，以及stable frontend identity/fingerprint、loader/compiler canonical persistent semantic products、loader-owned settled provider-demand plan和完整diagnostic check certificate。source module finalization及LLVM emission均通过同一persistent executor与资源预算；ready partition以bounded priority task进入剩余finalization工作之前，LLVM task共享唯一owned readonly whole-program index，独占LLVM context/target/output，并按partition提交顺序归并。Driver在对象写盘时保留stable key/fingerprint，Linker直接消费唯一ordered typed input product；Driver在临时object落盘前按完整link identity恢复最终结果，旧source/request manifest executable cache已删除。module program facts只从canonical item signatures投影；provider fixed point从partial graph三轮收敛为loader验证manifest后的一次完整图注入，最终check certificate让clean与non-clean warm check都只验证loader/source/graph/cache identity，semantic/body execution归零。Phase G和H acceptance均关闭；Phase I必须按五项逐项关闭，不能再用panic-flow内部进度代替整个阶段进度。
...
进展（2026-07-24）：G-6e5n 正式删除生产LLVM IR入口的`CodegenProgram/BackendLowering` aggregate barrier。新增`LlvmIrReadinessEmitter`封装唯一LLVM coordinator、prepared membership、确定序output/diagnostic汇总与declaration-only validation；Driver `emit_llvm_ir(request)`现在先取得tracked `CodegenPreparation`，随后在`with_backend_finalization_schedule` scoped callback内持有emitter，每次`wait_next`立即执行`ready event → index publish → pending retry → LLVM emission`，其余backend finalization worker可继续并行运行，callback结束后才做aggregate finish与optimization report归并。owner manifest从`BackendItemPlanFinalization`起即由唯一`Arc`控制块持有，schedule、emitter与最终`BackendLowering`只clone handle，不复制entries或建立借用/拥有双轨。CLI `emit --llvm`已物理删除显式`codegen → emit_from_codegen`两阶段调用，warnings与`--opt-report`改由live artifact携带的preparation diagnostics、optimization policy/report输出，不会为展示信息重新触发aggregate query。期间补齐instance `arg_module_id`的exact context dependency，避免partial fingerprint访问未发布module；native instance symbols改为带稳定source-context discriminator，杜绝LLVM按partition局部`.N`重命名造成的链接断裂，complete preparation结果也统一按stable key排序。两模块Driver回归锁定`codegen_preparation=1`、`backend_lowering=0`、`backend_module_finalization=2`并成功发射LLVM；LLVM 195项与两个跨模块CLI build回归通过，CLI detail timing锁定不存在独立codegen stage与aggregate backend-lowering query。native object/executable入口尚未迁移，故Phase G仍需下一切片把同一live protocol扩展到object cache/builtins/link inputs并删除剩余barrier。

进展（2026-07-24）：G-6o 将同一typed readiness protocol扩展到native object与executable入口。新增`LlvmNativeObjectReadinessEmitter`，每次`BackendModuleReady`立即执行唯一index publication、exact membership retry与`emit_native_object_partition`，object cache命中/失效统计、typed `IncrementalLinkInputs`和稳定排序均在同一emitter内汇总；完整index只在`finish()`阶段用于declaration-only校验与compiler builtins符号计算，未建立第二套membership或fallback。Driver `emit_native_objects(request)`现在直接消费tracked `CodegenPreparation`与scoped `BackendFinalizationSchedule`，不再先调用`codegen()`；`link_executable`复用该live object flow，CLI `emit --obj`/`emit --exe`也删除显式codegen与native双阶段，优化报告和warnings由live artifact携带。新增回归锁定native路径`codegen_preparation=1`、`backend_lowering=0`、finalization按ready module计数，并通过object cache、compiler builtins、CLI object/report/link参数测试；LLVM 195项、Driver 489项、CLI定向回归与fmt/diff检查通过。native partition当前沿readiness consumer同步发射，后续可在不改变typed ownership的前提下细化task overlap与link-result telemetry；Phase G剩余工作转为该并行化及最终全量 acceptance。
...
进展（2026-07-25）：G-6r 在clean commit `484d4b81`上完成最终3× fixed workload acceptance，并关闭Phase G。`module_backend`中位wall `2.592s`、CPU `113.7%`、RSS `262,176,768` bytes，finalization live-window peak growth `5,294,225` bytes；small CGU中位wall `22.96ms`、CPU `139.9%`、RSS `60,526,592` bytes、Rust peak-live `1,271,908` bytes；large CGU中位wall `193.26ms`、CPU `159.3%`、RSS `179,060,736` bytes、Rust peak-live `71,996,528` bytes。small/large全部6次样本都由runner强制验证`llvm.units=4`、`worker_lanes=2`、`memory_permits=4`、`ready_task_submissions=4`，没有同步回退、unit退化或资源身份变化；small首轮wall抖动由3次中位策略吸收，不用于阈值判断。结合G-5d/G-5e已经完成的拆分前后同机对照，本轮验证的是最终代码形状的稳定资源合同，不再要求另一个历史dirty revision冒充baseline。跨main branch的长期trend artifact仍是Phase A的CI职责，不是CGU ownership/overlap的未完成实现；因此G从约99%关闭为100%，不改动codegen代码或新增另一套telemetry。
...
5. corruption physical retirement 与 correctness verification mode。
...
进展（2026-07-24）：H-2a 建立三个互不混用的16-byte、versioned frontend content fingerprint域：`SourceContentFingerprint`精确覆盖source bytes，`SyntaxFingerprint`覆盖lossless syntax source但排除`SourceId`/`SourceRevision`，`ItemSignatureFingerprint`从完整module source中以固定marker替换top-level、trait default与extension method的function body span后编码其余lossless bytes。由此普通body内容变化仍会改变source/syntax identity，但不会污染source-level item signature identity；参数/返回类型、属性、using、nominal定义、binding initializer等其余文本继续保守参与signature fingerprint。实现复用正式`QueryFingerprintBuilder` canonical byte encoder并开放明确的parts重建边界，不使用`Debug`、随机`Hash`、span数值、node key、symbol slot或session-local handle。不同`SourceVersion`同文本、三域domain separation、三类function body编辑稳定及signature类型编辑失效均有回归。该切片尚未把fingerprint冒充serialized product，也未让包含revision-owned node key的当前query value跨session green；下一切片必须先组合cache schema/compiler/target/options namespace，再选择不含local handle的首个高价值product。Phase H现约20%，整份roadmap约95%。

进展（2026-07-24）：H-2b/H-4a 建立frontend cache namespace与三类product key的唯一typed组合协议。`FrontendCacheNamespace`以独立domain覆盖显式schema version、compiler package version、`TargetConfig`全部六个字符串字段与pointer width，以及影响frontend executable语义的`RuntimeModel`；optimization不参与frontend namespace，因为它不改变source/syntax/signature语义，后端已有独立policy fingerprint。`Frontend{Source,Syntax,ItemSignature}CacheKey`分别以独立domain组合namespace、canonical `StableModuleKey`路径与对应typed content fingerprint，三者均为16 bytes且不能在类型层互换；body-only编辑会使source/syntax key变化但保持item-signature key，module identity、target任一字段或runtime变化都会产生不同key。回归覆盖全部namespace维度、product domain separation、module separation、parts roundtrip与size。当前只定义cache addressing contract，尚未落盘任何包含revision-owned node/span/local handle的query payload；后续serializer必须在entry内重复编码并校验stable identity、namespace和content components，不能只信文件路径hash。Phase H现约30%，整份roadmap约95%；下一切片选择首个高价值、无local handle的frontend product并定义entry format、atomic publish、corruption miss与verification mode。

进展（2026-07-24）：H-3a/H-5a 落地首个真实persistent frontend product：loader-owned `ProviderSummary` cache。产品审计确认summary只含稳定content-hash `SymbolId`、布尔值与确定序vectors，不含span、`SourceVersion`、node/module/type-store handle；专用typed key组合namespace、stable module identity与精确source fingerprint。entry在payload外重复编码并验证magic/schema path、完整key、namespace、canonical module path、source fingerprint、payload length与独立checksum；decode对payload和sequence长度设硬上限，任何截断、尾随bytes、非法tag/boolean、checksum或header/path不一致都作为corruption删除并fresh重算。publish使用create-new staging、`sync_all`与rename原子发布，不建立旧schema fallback。`LoadRequest`与`DriverConfig`把同一artifact cache root接入loader；普通命中直接decode summary并跳过`ParsedModuleQuery`，verification模式则强制fresh recomputation，语义不一致时删除合法但错误的entry并发布fresh truth。跨独立query session回归锁定命中时`parsed_module`执行数从1降为0，损坏恢复后entry可再次命中，verification修复错误空summary后普通模式继续零parse复用。该产品目前按精确source key失效，尚未声称body-only provider summary复用；下一切片应把serialized dependency manifest接到item-signature key，并选择module declarations或definition/signature product，使第二次check开始出现可测的整阶段收益。Phase H现约45%，整份roadmap约96%。
...
进展（2026-07-24）：H-3c/H-5c 落地第二个无local handle的persistent frontend product：declaration-only `ModuleFacadeFacts`。entry header重复校验专用key、namespace、canonical module path、item signature与effective module-map fingerprint；payload只编码排序后的public type `SymbolId`、public reexport与provider source `UsedModulePath`，覆盖四类root和全部processing枚举，不写span、`SourceVersion`、node/module handle。decode保持64 MiB entry上限和sequence上限，并拒绝非法tag/bool、非canonical排序/重复、截断、尾随bytes、checksum或header/path不一致；atomic publish与corruption删除复用唯一cache实现。provider summary与facade facts现在共同消费一套source input/manifest/fresh-signature helper，不再各自维护source→signature truth path。跨session manifest+facade双命中可让provider discovery直接跳过候选module parse；body-only编辑只重建source manifest后复用同一facade product，signature或effective module-map变化则产生独立key。verification可修复格式合法但语义错误的facts，回归还锁定module-map package/local分类隔离与完整path-processing roundtrip。loader-query 58、compiler-query 156、Driver 489项及严格Clippy/fmt/diff check全部通过。完整body dependency仍未挂到signature域；下一切片应把`ModuleDeclarations`拆成exact-source body dependency summary与signature-stable declaration summary，再评估哪一层能实际减少graph discovery和后续parse。Phase H现约65%，整份roadmap仍约96%。

进展（2026-07-24）：H-3d/H-5d 持久化完整loader module dependency summary，但严格限制在exact-source域。审计确认`ResolvedModuleDeclaration`与explicit import携带raw source `Span`，function body长度变化会移动其后声明位置；qualified path、local using、iterator/provider demand与used alias又直接来自body，因此新增`FrontendModuleDependenciesCacheKey`只组合namespace、stable module、精确source fingerprint与effective module-map fingerprint，绝不使用item signature。entry额外重复记录source byte length，payload覆盖resolved declaration的name/visibility/span、package roots、完整`UsedModulePath`、explicit imports及used aliases；decode验证四类visibility、path processing tag、canonical sorted/unique集合、重复declaration name和所有span边界。只有read、parse、target prune与declaration resolve四层均无诊断时才publish，malformed source和duplicate declaration不会产生可复用entry。普通命中让`ModuleDeclarationsQuery`在graph discovery阶段零parse返回；body长度编辑必定换key并fresh重算，回归证明后置module declaration span同步变化。corruption删除、module-map隔离与verification修复合法但错误entry均已覆盖。该优化尚不意味着完整check零parse：最终selected `LoadedModule`仍需要当前revision item tree。loader-query 63、compiler-query 156、Driver 489项及严格Clippy/fmt/diff check全部通过；下一切片应量化跨session graph/provider discovery命中，再审计不含span/node/type handle的compiler declaration/public-surface产品。Phase H现约72%，整份roadmap约97%。

进展（2026-07-24）：H-3e 用两套独立`SourceDatabase`与`QueryDb`建立真实graph-only跨session验收：首次从入口递归发现三模块时执行三次`ModuleDeclarationsQuery`并生成三份持久summary，第二session以相同source重建完整graph时仍执行三次typed declaration query，但`ParsedModuleQuery`执行数严格为0；该指标只覆盖graph discovery，不把最终`LoadedModule`为当前revision构建item tree所需的parse混入。验收同时发现并修复此前单模块产品测试未暴露的symbol residency缺口：只序列化稳定`SymbolId`并不足以让新session恢复模块路径，因为新的`SymbolTable`无法从hash反查文本。所有包含symbol的provider summary、facade facts与module dependencies payload现在都先编码按ID排序的完整symbol text dictionary，decode校验text stable hash、ID唯一性以及dictionary与产品实际引用集合完全相等，随后才把文本intern进当前session；缺失文本的fresh产品拒绝publish。frontend cache直接升级到唯一v3目录和新magic，完全不读取v2或旧payload。独立roundtrip回归证明load会恢复symbol text，三模块graph路径不再退化为占位符。loader-query 64项通过；下一切片审计compiler declaration/public-surface产品并拆出可持久化、无local handle/span的稳定投影。Phase H现约76%，整份roadmap约97%。

进展（2026-07-24）：H-3f 完成compiler public-surface产品的深度handle审计并建立可持久化的单模块输入边界。最终`ModulePublicSurface`/`ModuleUsingScope`不能直接按单模块signature落盘：它们包含当前graph owner的`ModuleId/GlobalDefId`，且re-export/using传播结果依赖跨模块闭包；其中name/directive span又要求exact-source身份。新增`PublicSurfaceModuleFacts`只提取public-surface算法实际读取的稳定`DefId`、name/kind/parent/visibility/span、确定序module scope与enum variants，以及source-order module usings，不携带`ModuleId`、`SourceVersion`、node key、generic AST/type node或`DefNodeMap`。`materialize_for_public_surface(current_module_id)`只在当前owner下重建专用`DefCollection`计算视图；独立allocator回归证明同一facts可rebase到不同module owner且facts往返不变。compiler新增`PublicSurfaceModuleFactsQuery` semantic boundary，`PublicSurfacesQuery`与`PublicUsingScopesQuery`改为只消费该边界并在当前graph中重建，原始`ModuleDefsQuery`仍服务其余需要revision-local node/generic数据的消费者。该切片尚未声称跨session命中：facts仍由当前`ModuleDefsQuery`fresh生成；下一切片应由loader按exact-source持久化该DTO并通过`LoaderFactProvider`提供，使compiler边界命中时跳过定义收集，同时继续由aggregate算法在当前owner解析跨模块关系。nia-defs 7、compiler-query 156项及严格Clippy/fmt/diff check通过。Phase H现约80%，整份roadmap约98%。
...
进展（2026-07-24）：H-3h/H-5e 将唯一loader-owned `PublicSurfaceModuleFacts`正式接入v3 persistent frontend cache。entry header重复校验专用key、namespace、canonical module path、精确source fingerprint、source byte length、payload length与独立checksum；payload编码稳定`DefId`、全部`DefKind`/visibility、parent、definition span、三类module scope、enum scope和完整递归using syntax，并携带与实际引用集合完全相等的symbol text dictionary供新session rehydrate。encode/decode共同拒绝非canonical或重复def/scope、错误kind/parent/enum关系、alias/span不配对、过深selector、越界span、非法tag、截断、尾随bytes和header/path/checksum不一致；任何corruption会物理删除后fresh重算，read/parse/prune/definition collection任一诊断都禁止publish。普通命中在该query边界直接跳过graph lookup、full active item tree fact与definition collection；verification强制fresh并替换格式合法但语义错误的entry。独立`SourceDatabase`/`QueryDb`回归证明第二session facts query执行一次而`loader_active_module_item_tree_fact`与`parsed_module`均为0，symbol文本可解析；corruption、exact-source body长度编辑、后续复用和诊断不落盘均有验收，direct roundtrip覆盖全部definition/using稳定字段。这里不把最终`ModulePublicSurface`/`ModuleUsingScope`跨模块闭包持久化，也不声称完整check已零parse：生产Driver使用query-backed `LoaderDatabase`，其他revision-local item-tree/origin消费者仍会触发parse；本切片在完整pipeline中的直接收益是消除public-surface链的重复definition collection。loader-query 68项、compiler-query 156项及严格Clippy通过。Phase H现约90%，整份roadmap约98%；下一切片应以真实第二次check trace/perf量化各剩余parse/definition/signature成本，再只选择能穿透当前loader fact边界的stable product或调整消费顺序，不能用孤立query命中替代端到端收益。

进展（2026-07-24）：H-6a 建立真实跨进程warm check入口和首轮端到端成本审计。`nia check`新增唯一`--cache-dir <path>`/`--cache-dir=<path>`参数并直接配置既有`DriverConfig.artifact_cache_dir`，没有另建check专用cache、默认隐式目录或兼容alias；`--opt-report`也复用同一configured Driver。timing counter现在显式列出parse、loader active tree、module dependencies/provider/facade/public-surface facts以及compiler module-def/public-surface query executions；审计同时修复Driver过去只汇总compiler DB trace而把共享session loader执行全部报告为0的问题，现在总数与分项都组合两个typed DB各自trace，不复制query state。release compiler对`examples/modules/main.nia`使用全新cache目录做两次独立进程check：总query executions 9898→9823，`loader_public_surface_module_facts` 110→55，`parsed_module` 148→138；但active item-tree facts保持265、`module_defs`保持158、compiler public-surface facts保持158。单次wall 2.49s→2.36s只作观察、不作性能结论；稳定的execution差分证明现有产品有效，也证明收益仍被query-backed revision-local item-tree/origin facts和深层semantic链限制。Phase H仍约90%、整份roadmap仍约98%；下一切片先审计这些loader facts的真实消费者和可重建边界，再决定stable syntax/item-tree投影还是更深semantic product，不能仅继续堆叠在parse之后才消费的cache。

进展（2026-07-24）：H-6b 深审计确认生产Driver从未通过eager `LoadedProgram`喂给compiler，而是把共享同一`QuerySession`的`LoaderDatabase`作为`LoaderFactProvider`，按module拉取parse errors、item tree、active item tree与node origins；先前roadmap中的eager判断已更正。曾实现并完整验证exact-source parse-clean marker：独立query第二session可做到零parse，corruption/verification/错误不落盘语义也正确；但同一release端到端cold/warm check的全部计数与未实现marker时逐项相同，因为每个clean module随后都被active tree/origin消费者解析。该实现因此在提交前整体删除，不保留typed key、entry、reader、兼容schema或无收益测试。`ModuleItemTree`/`ActiveModuleItemTree`本身携带`VersionedNodeKey`和完整AST，直接序列化会泄漏revision owner；仅缓存clean状态或parse diagnostics又不能替代后续消费者。timing counters现扩展到full defs、module/active tree、full/signature type resolution/lowering/item signatures/normalization与signature layouts；新一轮独立进程cold/warm数据中module/active tree与module defs各158次、full defs 67次、full type resolution/lowering/normalization各49次，全部不变，而signature type resolution、lowering与item signatures各489次、signature normalization 141次，同样完全不变。由此下一高价值边界收敛为signature semantic bundle及其canonical type/def rehydration，而不是parse/item-tree小缓存；不再接受只改善孤立loader query的产品。Phase H比例不因被否决实验上调。

进展（2026-07-25）：H-3i/H-5f 落地首个compiler-owned persistent semantic product：`SignatureTypeResolution`。新16-byte typed key组合frontend namespace、目标module的canonical `StableModuleKey`、五类`SignatureItemSet`和按stable path排序的完整loaded-program exact-source universe；它保守覆盖所有跨模块public surface/using/graph语义，任何source byte或长度变化都产生新key，不把body-insensitive身份错误用于仍携带raw span的node locator。payload不保存`SourceId`、`SourceRevision`、`ModuleId`、`NodeId`或symbol slot：`NodeSite`只编码kind与span/child path，`GlobalDefId`编码stable module path与`DefId`，const-generic symbol编码text；load时统一映射到当前`SourceVersion`、current graph owner、`NodeStore`与`SymbolTable`。program universe先从stable module sequence映射current loader graph，owner更换时不会枚举semantic-green旧snapshot。entry重复校验magic、typed key、namespace、program fingerprint、module、item set、source length、payload length与checksum；decode拒绝越界span、重复node、未知module、非法enum/tag、过大sequence、截断和尾随bytes，corruption立即物理删除。含诊断resolution不落盘；verification强制fresh resolver并替换格式合法但语义错误的entry，不保留旧schema reader、fallback artifact或双轨truth。跨独立`SourceDatabase`/`QuerySession`回归证明普通命中删除`signature_type_resolution → module_defs`依赖，verification恢复fresh边；direct roundtrip证明source/module/symbol/node owner全部rebase，corruption会退休文件。release `examples/modules/main.nia`在全新cache的独立进程cold/warm为总query executions `9929 → 9689`，warm相对H-6a的`9823`再少134；显式counter记录489个typed demand中487个持久命中、2个含诊断miss，单次wall `2.383s → 2.219s`仅作观察。外层`signature_type_resolution` query仍应执行489次以保留typed demand/dependency记录，命中只跳过resolver工作；type lowering、item signatures与normalization尚未canonical落盘，因此Phase H约93%、整份roadmap仍约98%。下一切片应建立canonical type graph/`InternedTyId` rehydration并把signature lowering/signatures纳入同一semantic bundle；在node locator仍依赖精确span前，不宣称body-only本module复用。

进展（2026-07-25）：H-3j/H-5g 将`SignatureTypeLowering`接入第二个compiler-owned persistent semantic product，并建立首个canonical type graph codec。端到端审计先确认489次typed lowering demand中469次（约95.9%）没有const-expression owner，只有20次包含array length/const generic expression；因此首版只发布diagnostics、`const_exprs`与`const_expr_summaries`均为空的完整`type_uses`，任何藏在`ArrayLenTy`或`ConstGenericValue`中的`GlobalConstExprId`也由codec再次硬拒绝，绝不序列化revision-owned `Expr` AST、建立兼容payload或把不完整产品当命中。独立16-byte lowering key、magic、目录和checksum与resolution产品完全分域，但共同保守依赖frontend namespace、stable module、五类`SignatureItemSet`及完整exact-source program universe。payload把按stable `NodeSite`顺序遍历的可达`TyKind`编码为child-first graph-local index；nominal/source trait owner编码canonical module path+`DefId`，symbol编码text，全部primitive、pointer/slice/array/vector/range/function/optional/error union/nominal/builtin trait/type/trait object/projection/generic/self类型以及非表达式const generic值均有固定tag。load只在current graph解析module owner、向current `TypeStore`重新intern并恢复current symbol/source owner；非法back-reference、未知module、越界node span、重复type-use、非法tag/bool/char、截断/尾随或checksum错误均删除artifact。普通lowering命中发生在`SignatureItemTreeQuery`和`SignatureTypeResolutionQuery`之前，verification则强制fresh lowering并修复或删除语义错误entry；跨独立session回归把warm `signature_type_resolution` execution锁定为0，复杂type graph重新编码逐字节相同，corruption和const-expression拒绝均有覆盖。release `examples/modules/main.nia`全新cache独立进程cold/warm中，lowering hit为`162 → 469`、const-expression fresh miss稳定为20，`signature_type_resolution` executions为`328 → 21`，总query executions为`9768 → 9207`；相对上一切片warm `9689`再减少482次。外层`signature_type_lowering`仍执行489次以保留typed demand，命中跳过lowerer及resolver；`SignatureItemSignatures`与normalization尚未持久化。compiler-query 160、loader-query 69、Driver 489项以及workspace all-target/all-feature严格Clippy、fmt/diff check通过。Phase H约96%、整份roadmap约99%；下一切片优先审计并持久化不含span/node owner的`ItemSignatures`稳定投影，使warm chain继续穿透signature item tree/defs，随后再评估normalization是否仍有独立收益。

进展（2026-07-25）：H-3k/H-5h 将完整`SignatureItemSignatures`接入第三个compiler-owned persistent semantic product，并把H-3j的canonical type graph codec提升为lowering/signatures共享的唯一实现。独立16-byte typed key、magic、目录和checksum继续组合frontend namespace、stable module、五类`SignatureItemSet`及完整exact-source program universe，不读取旧schema、不建立fallback artifact或第二truth source。payload覆盖function、struct/union、trait/trait impl、enum、type alias、global/const及generic、where predicate/binding、visibility、receiver、attribute、builtin tag和全部span；所有`InternedTyId`经同一child-first canonical graph编码，nominal owner与symbol在load时分别映射到current graph和current symbol/type owner。decode拒绝越界span、非确定序或重复top-level/nested def与impl ID、非法tag/bool/char、type graph back-reference、未知module、截断和尾随bytes；corruption立即物理删除，任何diagnostic或const-expression lowering产品均禁止publish，verification强制fresh收集并替换格式合法但语义错误的entry。跨独立session回归证明普通signatures命中只保留`frontend_program_sources`依赖，直接跳过signature item tree、module defs与该需求的type lowering；verification恢复完整fresh依赖。direct roundtrip覆盖全部signature category、current `TypeStore`/module/symbol rehydrate、canonical re-encode与corruption retirement。release `examples/modules/main.nia`全新cache独立进程cold/warm中，signatures hit为`162 → 469`，20个const-expression case明确保持fresh；总query executions为`9624 → 8652`，相对上一切片warm `9207`再减少555次，signature lowering executions为`396 → 180`、resolution为`328 → 21`，loader active item-tree facts为`265 → 213`。这证明收益已穿透item-tree/defs/lowering链而非孤立query命中；剩余lowering主要来自normalization等直接消费者。compiler-query 161、loader-query 69、Driver 489项以及release build、workspace all-target/all-feature严格Clippy、fmt与diff check通过。Phase H约98%、整份roadmap约99%；下一切片应审计并持久化signature normalization，或先调整其直接lowering消费顺序，以端到端execution差分决定，不缓存无独立收益的中间marker。

进展（2026-07-25）：H-6c 完成signature normalization与其真实消费者的端到端否决审计。实验曾完整实现独立typed key、entry/checksum、canonical normalized type-pair graph、current module/type/symbol owner rehydrate、corruption retirement、verification、const-expression拒绝及跨session依赖回归；真实warm check可命中135/141个normalization demand，但总query executions仍为`8652`、signature lowering仍为180，与无该artifact时逐项相同。有无normalization目录的交替warm样本中provider/check时间落在同一噪声区间，原因是extension/program-signature等实际消费者本来就同时读取raw lowering、item signatures与normalization，缓存最后一项既不能切断前两项也没有稳定CPU收益。该artifact因此在提交前连同key、magic、目录、reader、counter和测试全部物理删除，不保留旧schema或fallback。第二个实验让nominal provider facts从完整provider facts投影，虽然删除重复收集代码，却使三个只需nominal projection的module被迫构建完整methods/associated-value产品，总executions反而`8652 → 8655`，也已整体删除并保留原有按需粒度。唯一保留的是现有Driver timing汇总新增六个精确counter：warm `module_program_signature_facts=97`、`extension_signature_module_input=140`、`extension_trait_solving_module_facts=140`、`extension_provider_module_facts=134`、`extension_provider_nominal_module_facts=50`、`extension_provider_validation_facts=20`。这些数据把下一高价值边界上移到可独立按需恢复的extension semantic facts，而不是normalization或强制合并nominal/full产品；新产品必须直接替代其raw defs/lowering/signatures/normalization依赖并保持现有projection粒度。Phase H与整体比例不因两个被否决实验上调。

进展（2026-07-25）：H-3l/H-5i 持久化首个完整extension semantic产品`ExtensionTraitSolvingModuleFacts`。独立16-byte typed key、magic、目录和checksum组合frontend namespace、stable module与完整exact-source program universe，不复用signature item set身份，也不读取旧schema或fallback artifact。payload覆盖有效`ProgramTraitImplSignature`、无效trait impl method的稳定`GlobalDefId`集合、impl/builtin/generic、target/trait type与const args、where predicate/binding、associated type/value、visibility、span和symbol text；所有type handle继续复用唯一canonical type graph codec，source trait/nominal owner按stable module path编码，load时统一rebase到current graph的`ModuleId`、current `TypeStore`和`SymbolTable`。decode拒绝错误module owner、重复impl/def/name、越界span、非法type graph、未知module、截断和尾随bytes；corruption物理删除，任何lowering/signature/normalization诊断或const-expression输入都禁止publish。普通命中只读取`FrontendProgramSourcesQuery`并直接跳过`ExtensionSignatureModuleInputQuery`及其raw defs/lowering/signatures/normalization链；verification强制fresh重算并替换格式合法但语义错误的entry。跨独立session回归将warm fixture的signature resolution/lowering execution都锁定为0，并验证普通命中不存在raw-input依赖、verification恢复fresh边；direct roundtrip证明module/type/symbol owner rehydrate、跨模块trait/nominal owner、canonical re-encode与corruption retirement。release `examples/modules/main.nia`全新cache的cold/warm中，总query executions为`9475 → 8153`，相对上一切片warm `8652`再少499；extension facts命中`47 → 134`，raw extension input `106 → 19`，signature lowering `377 → 123`，signature normalization `121 → 81`。这是真实穿透raw consumer链的收益，不是孤立cache hit。compiler-query 162、loader-query 69、Driver 489项以及workspace all-target/all-feature严格Clippy、fmt/diff check全部通过。Phase H约99%、整份roadmap仍约99%；下一切片应分别审计`ExtensionProviderModuleFacts`、nominal facts与validation facts的稳定字段和真实dependency cut，继续保持现有按需粒度，不能重新强制合并full/nominal产品。

进展（2026-07-25）：H-6d 对剩余full provider与nominal provider边界分别完成了可运行artifact和端到端否决审计。full provider实验使用独立typed key/schema/checksum，稳定编码method、associated value、nominal projection及canonical type/symbol/module owner，普通命中可直接切断其`ModuleDefs`、signature lowering/signatures/normalization依赖，verification、diagnostic/const-expression拒绝、owner rehydrate、canonical re-encode、corruption retirement和跨session回归均通过；真实warm可命中128/134个demand，但总executions仅`8153 → 8150`，signature lowering仍为123，只有signature normalization `81 → 78`。该产品因此连同key、magic、目录、reader、counter和测试完整删除。随后独立nominal-only实验只编码stable target module path、`DefId`和visibility，50/50个warm demand全部命中并跳过自身raw链，但端到端仍恰好为`8150`、lowering仍为123、normalization仍为78；这证明full与nominal边界切掉的是同一组会被validation、program-signature等其他consumer立即重建的输入，而不是两个可叠加的work product。nominal artifact同样整体删除，不保留旧schema、兼容reader或孤立hit telemetry。两次实验的cold均为9474，与H-3l基线9475等价；wall差异只作噪声观察。Phase H与整体比例不因被否决artifact上调。下一切片应审计20次validation facts及其全局trait index/impl callback依赖，或先重排共享consumer使一个更高层产品能同时消除重叠raw链；不能继续按单模块叶子query堆cache。

进展（2026-07-25）：H-6e 对97次`ModuleProgramSignatureFactsQuery`需求完成共享consumer审计，并删除其错误的raw依赖而不新增第五种持久cache。`ModuleProgramSignatureFacts`实际只需要canonical item signatures中的函数签名和trait定义集合，原先显式读取的`ModuleDefsQuery`与`SignatureTypeLoweringQuery`均未提供独有语义；`FunctionSignature`因此纳入函数名，使名称与其余函数签名字段由同一个`SignatureItemSignatures`产品发布，trait定义则直接从`signatures.traits`的key投影。program facts provider和executable function收集器不再查询defs，`ModuleSignatureInput.defs/lowering`整体删除；依赖图回归锁定program facts只依赖`signature_item_signatures`而不依赖defs/lowering。item-signature持久payload的唯一magic升级为`NIASI002`并直接编码函数名，旧`NIASI001`不读取、不迁移，命中即按corruption retirement规则删除。release `examples/modules/main.nia`全新cache独立进程cold/warm中，总query executions为`9453 → 8125`；相对H-3l基线cold `9475`少22、warm `8153`少28，signature lowering分别从`377 → 355`和`123 → 95`，normalization warm保持81。这证明收益来自删除重复consumer边而非孤立cache hit。compiler-query 162、loader-query 69、Driver 489项及workspace all-target/all-feature严格Clippy、fmt/diff check全部通过。Phase H仍约99%、整份roadmap仍约99%；下一切片集中审计20次validation facts与其全局trait index/impl callback，判断能否形成最后一个同时替代重叠raw链的高层产品，或以无剩余独立收益的证据关闭H。

进展（2026-07-25）：H-6f validation边界审计首先删除了一个无消费者的伪产品。`ExtensionProviderValidationFacts.methods`在生产路径和测试中均无人读取，最终诊断收集只消费`diagnostics`；分析入口因此改为diagnostics-only，不再为每个有效impl复制trait args/where predicates、计算effective generics或构建四份`ExtensionMethods`索引，随之失去用途的`impl_trait_args` helper也已物理删除。release同一全新cache的cold/warm executions保持`9453 → 8125`，证明此步是CPU/分配路径清理而非query-graph剪枝，不据此上调完成度。compiler-query 162项、program-signatures及workspace all-target/all-feature严格Clippy、fmt/diff check通过。剩余validation输出已收缩为稳定的`NAME_RESOLUTION` span+summary序列，不携带type/module/symbol owner；下一步将以独立typed artifact实测它能否切断19次warm raw extension input及全局trait validation链，无端到端收益则同样整体删除。

进展（2026-07-25）：H-3m/H-5j 持久化diagnostics-only的`ExtensionProviderValidationFacts`最终产品。独立16-byte typed key、`NIAEV001` magic、目录和checksum组合frontend namespace、canonical module path与完整exact-source program universe，不复用trait-solving key、不读取旧magic或建立兼容reader。payload只接受当前validation实际产生的registered `E0201` user error：恰好一个source primary label、label message等于summary且无notes/help/related/debug；仅编码checked source范围内的span和UTF-8 summary，不携带`ModuleId`、type handle、symbol slot、source revision或复杂diagnostic graph，任何形状扩展、越界、非法长度、截断、尾随、checksum或identity错误均拒绝并物理退休artifact。普通命中在module eligibility、`ExtensionSignatureModuleInputQuery`与全局trait index之前返回；verification强制fresh重算，语义不等则删除并替换唯一entry。跨独立Loader/Compiler session依赖回归锁定cold与verification恢复raw/global边，而warm只依赖`frontend_program_sources`；独立Driver回归证明非空错误诊断在普通warm和verification中逐字段一致，direct codec回归覆盖非法形状拒绝及corruption retirement。release `examples/modules/main.nia`全新cache cold保持9453 executions，warm从H-6e的`8125 → 8093`再少32；20/20 validation demand命中，raw extension input `19 → 6`，signature item-signatures `228 → 211`，signature lowering 95与normalization 81不变。这是可独立切断共享raw/global链的端到端收益，产品保留。compiler-query 163、loader-query 70、Driver 490项及workspace all-target/all-feature严格Clippy、fmt/diff check通过。Phase H仍约99%而非100%：extension边界已收口，但原始acceptance要求无改动check接近cache validation成本，当前8093次execution的剩余主体已转到body/const/layout等广域semantic链；下一步必须按新的execution/timing profile选择跨域高层产品或消费重排，不能再堆extension叶子cache。

进展（2026-07-25）：H-6g 将warm acceptance telemetry从signature/extension扩展到const、layout/check及item-owned executable产品，使用Driver现有loader+compiler query trace汇总固定counter，不新增环境变量、独立状态或调试执行路径。同一H-3m warm样本仍为8093次总execution，其中普通module `body_check/layouts/flow/static/abi`均为0，`const_module=32`且array-length/enum/value汇总各1；真正的item链是`executable_function_body=85`、`executable_static_init=84`、`executable_value_ref_item=242`与`executable_value_ref_edges=242`，`executable_checked_module_facts/modules`各1。timing同时显示主要wall落在item body check反复构造const inputs和lower module，但审计不应直接从最大产品开始：`TypedBody`覆盖大量type/global/local/callee/aggregate/asm等variant，稳定序列化面很大；相反`ExecutableValueRefEdges`只发布function/global两个`GlobalDefId`集合，却在每次重算时重新切item tree并执行value/local resolution和semantic-use收集。下一H切片因此优先建立per-stable-def的value-ref edge产品，要求普通命中直接跳过item locator和resolver链并以端到端execution/timing证明收益；完成后再用剩余85/84个body/static需求决定是否建设canonical TypedBody/StaticInit codec。Phase H与整体比例不因新增telemetry上调。

进展（2026-07-25）：H-3n/H-5k/H-6h 持久化per-stable-def的`ExecutableValueRefEdges`最终产品。独立16-byte typed key、`NIAER001` magic、目录和checksum组合frontend namespace、owner canonical module path、owner `DefId`与完整exact-source program universe，不读取旧magic、不建立fallback或第二truth source。payload仅包含按canonical module path+`DefId`排序去重的function/global边；不保存`ModuleId`、node/source revision、symbol slot、type handle或body IR，load时必须把每个module path映射到current graph owner，未知module、重复/非递增集合、非法长度、截断、尾随、identity或checksum错误均拒绝并物理退休artifact。普通命中发生在`ExecutableValueRefItemQuery`、item tree切片和value/local resolver之前；verification强制fresh计算，语义不等时删除并替换唯一entry。直连跨独立compiler session回归锁定cold/verification恢复item依赖、warm只依赖`frontend_program_sources`，并注入checksum/identity均合法但语义为空的错误entry，证明verification替换后下一warm恢复current `ModuleId`上的正确callee；direct codec回归覆盖跨session module owner rehydrate与合法checksum的重复集合退休。release `examples/modules/main.nia`全新cache cold/warm为`9430 → 7805` executions；相对H-3m基线cold `9453`少23、warm `8093`少288，warm edge命中`242/242`，`executable_value_ref_item`从242降为0，signature item-signatures `211 → 209`，lowering 95与normalization 81不变；同次样本`executable_checked_modules.value_ref_edges`聚合时间从cold 183.8ms降为warm 6.2ms，单次总wall `2.480s → 2.194s`只作观察。这证明小型稳定边产品切断了真实resolver链，予以保留。compiler-query 165、loader-query 70、Driver 490项及workspace all-target/all-feature严格Clippy、fmt/diff check通过。Phase H与整体仍约99%而非100%：剩余主体是85个`ExecutableFunctionBody`、84个`ExecutableStaticInit`及其共享const/body输入；下一切片先审计两类产品的重叠生产链、稳定最小投影和消费顺序，再决定是否值得建设canonical body/static codec，不能仅因它们耗时最高就序列化完整`TypedBody`。

进展（2026-07-25）：H-6i 完成function body/static init共享生产链审计，并通过current-session产品重排删除最大重复算法。FactsOnly阶段原本已经产生每模块最终`ConstCheck`，其`ExecutableCheckCaches`也持有对应`ConstModuleLowering`；但85个`ExecutableFunctionBody`与84个`ExecutableStaticInit`随后丢弃这两个结果，每个item重新lower同一const module并重跑array-length/enum/value/typed四阶段。`ExecutableCheckedModuleFacts`现在只额外发布实际runtime owner module的current `Arc<ResolvedConstModule>`，不携带lowering diagnostics、AST副本或新的持久schema；BodyOnly/StaticInitOnly显式组合该module与`CheckedModule.const_eval`，缺失直接违反内部不变量而非fallback重算。runtime module集合过滤避免旧reachability cache进入当前产品，既有cache `retain_modules`继续负责owner回收；回归覆盖普通function、global、local static、forwarded array length、reexport trait witness，并断言每个runtime def的current module都有const product。release同一代表样本executions保持cold/warm `9430/7805`，但`executable_body_check.const_inputs`聚合从`3.129s/3.059s → 0.174s/0.172s`，其中lower-module从`2.456s/2.393s → 0.138s/0.138s`；user CPU从`4.529s/4.208s → 1.641s/1.390s`，单次wall从`2.480s/2.194s → 1.732s/1.484s`，wall仍只作观察。复用后85次BodyOnly lowering仅约21ms、84次StaticInitOnly约5ms，因此canonical `TypedBody`/`StaticInit`持久codec的巨大type/local/callee/aggregate/asm schema不再有合理收益，明确否决而不做实验artifact。compiler-query 165、Driver 490项及workspace all-target/all-feature严格Clippy、fmt/diff check通过。Phase H与整体仍约99%：warm主体已上移到46次`executable_fact_check`与两轮`executable_provider_demands`，下一步审计这两轮是否重复构造同一facts、能否以更高层稳定产品或worklist收敛关闭H。

进展（2026-07-25）：H-6j 将provider fixed-point telemetry细分为按轮次和typed request kind的固定counter，并完成剩余高层边界审计。同一warm样本三轮收敛中，第1轮`ExecutableProviderDemandsQuery`产生26项：1 method、1 trait impl、10 `ModuleSemantic`、14 `ModuleBody`，graph变化且必须使resolved body facts失效；第2轮产生33项：0 method/trait、15 semantic、18 body，graph变化但不使body facts失效；第3轮跳过discovery并完成编译。两轮不是同一结果的误调用，而是method/trait需求先加载provider、随后只有新图上的精确semantic/body reachability才可见；直接让loader eager展开所有新provider会检查额外body并改变诊断边界，禁止采用。compiler-owned persistent product也不能在首轮用“当前loaded program universe”作为key，因为它恰好缺少尚未由demand加载的provider sources。下一候选因此必须是loader-owned settled provider-demand plan：稳定入口/target/runtime索引指向列出最终source path+exact fingerprint的manifest，load先从`SourceDatabase`逐项验证未加载路径，payload把source/module path及method/trait symbol text编码为稳定请求；普通warm在compiler第一次discovery前一次注入完整plan，verification重跑fixed-point并在集合不等时退休、替换，corruption同样物理删除，不保留旧reader或把eager graph当fallback。该产品只有在端到端把三轮收敛降为一轮并显著削减`executable_provider_demands`/`executable_fact_check`时保留；否则删除全部schema。Phase H与整体仍约99%。

进展（2026-07-25）：H-3o/H-5l/H-6k 完成并保留loader-owned settled provider-demand plan。独立16-byte `FrontendProviderDemandPlanCacheKey`以唯一domain组合frontend namespace、canonical entry identity、effective module-map fingerprint与package-root graph policy；`NIAFPD01`唯一schema的manifest按canonical path列出最终graph全部source的exact content fingerprint与长度，并强制包含entry、覆盖每个demand的source及semantic/body module path。payload按稳定顺序编码四类typed request，method/trait symbol通过text dictionary保存并在load时重新intern到current `SymbolTable`，不落盘`SymbolId` slot、`ModuleId`、source revision或graph snapshot；checksum、非法长度/tag/path、非严格排序、重复、identity/path closure错误和corruption均物理退休，不读取旧magic或建立fallback plan。普通warm在首个`ModuleGraphQuery`前从`SourceDatabase`验证包括尚未加载provider在内的全部manifest source，并以`ProviderFactStore::with_initial_demands`建立current full snapshot，不伪造指向不存在predecessor graph的additive transition。发布只由compiler在整个provider worklist真正返回前调用显式`settle_provider_demands` hook，loader的单次`ProviderGraphUpdate::Stable`不再被误当成全局收敛；source replacement同步退休plan。verification模式安装symbol text但不注入candidate，完整fresh fixed point后集合不等即删除并原子发布唯一replacement。跨session回归覆盖四类request/current symbol恢复、source edit retirement、corrupt entry删除和格式合法但语义错误artifact的fresh replacement。release `check examples/modules/main.nia`全新cache cold/warm从`9430 → 4582` executions，provider rounds `3 → 1`，`executable_provider_demands` `2 → 1`，`executable_fact_check` `46 → 24`；同次user CPU `1.700s → 0.838s`、wall `1.916s → 0.871s`只作观察。相对H-6i的7805 warm execution再少3223，证明完整plan避免了partial graph查询及随后整批失效，达到实验保留门槛。compiler-query 166、loader-query 73、Driver 490项及workspace all-target/all-feature严格Clippy、release build、fmt/diff check通过。Phase H与整体仍约99%而非100%：4582次warm execution已大幅下降但仍不是纯cache validation，下一步必须按新profile审计剩余最高层dependency cut或以无独立收益证据定义H的关闭线，不能回退eager provider graph、增加plan fallback或继续堆叶子artifact。
...
进展（2026-07-25）：H-3p/H-5m/H-6n 落地并验收唯一`NIACC001` clean-check certificate，Phase H关闭为100%。独立16-byte typed key与entry header重复覆盖frontend namespace、canonical entry、check scope及完整check-input fingerprint；input按stable module path排序编码exact program-source universe、module path/parent/declaration visibility与target、executable root membership，以及settled provider-demand集合，不保存`ModuleId`、source revision、symbol slot、type/local/body handle或loader traversal flag。payload只有cold analysis产生的checked/reachable body telemetry标量；只有正式report完全无diagnostic时才publish，因此普通命中返回current graph snapshot/current optimization和空diagnostics，不伪造typed analysis，也不使warning/error消失。checksum、magic/header/key/input/scope不一致和截断均物理退休；verification强制fresh analysis，clean结果原子替换certificate，非空diagnostic则删除同identity旧证书。跨独立session回归锁定warm使用新的current graph owner、`entry_checked_program/executable_checked_module_facts/body_check=0`，source edit强制fresh且错误可见，verification完整重算后下一warm继续命中；graph visibility和provider demand分别改变input fingerprint。首次真实实验发现把`semantic_selected/process_*`瞬时loader状态写入key会导致第三次才命中，这些非语义字段已在提交前删除；全新cache最终第二次`check examples/modules/main.nia`从cold `9430`降至`770` executions，wall `1.692s → 0.019s`、RSS `250,863,616 → 38,010,880` bytes，certificate hit=1，全部compiler semantic/body/const/layout/signature execution为0，剩余770次只做loader source/graph/provider artifact validation。非空diagnostic持久化需要Phase I的stable diagnostic store/bundle，不能让H恢复完整diagnostic codec或旧typed output。compiler-query 170项、Driver 490项、release build与定向certificate 4项通过；Phase H的clean no-op与body-edit incremental acceptance至此完成。
...
进展（2026-07-25）：I-1c 将frontend foundation的module membership与definition层接入显式failure channel。可替换的`parse_ok_module_ids`/`semantic_module_ids`和`module_defs`/`full_module_defs` provider function pointer统一返回`QueryResult`；parse membership逐module传播`ModuleParseErrorsQuery`失败，semantic membership传播parse集合与graph失败，definition collector传播active/full item-tree失败。缺失module回归现覆盖`ModuleItemTreeInput -> ModuleItemTree -> ActiveModuleItemTree -> ModuleDefs`四层直接`Err`，provider override回归同步改为typed success而不恢复裸Value adapter。compiler显式provider增至24个，legacy裸provider降至110个；下一切片继续沿public-surface/type-resolution等DAG向check/codegen root推进。

进展（2026-07-25）：I-1d 完整迁移public-surface/using-scope/type-resolution前置图。`PublicSurfaces`、module surface、using scopes、type exposure及module/value/type/unresolved细粒度projection的provider与query全部返回`QueryResult`，批量defs/facts组装逐项传播失败；`FrontendProgramSourcesQuery`也改为显式传播module path/source version失败，缺失fingerprint或重复stable path仍是正常`Ok(None)`。full/declaration/signature/signature-const四类type-resolution provider不再在不可失败的lookup trait回调里调用panic型`get`：graph、public surfaces与module using scope预先以显式query取得并作为immutable lookup输入，跨模块defs继续按resolver实际访问惰性`try_get`，首次失败记录到本次调用私有failure slot，resolver返回后立即变为`Err`且不发布persistent signature artifact。缺失module回归已从input/item tree/defs扩展到type resolution；compiler显式provider增至41个，legacy裸provider降至93个。下一切片应沿type lowering/item signatures/normalization与value-resolution继续向check roots迁移，并最终删除lookup query adapter。

进展（2026-07-25）：I-1e 将type lowering、item signatures和normalization链完全迁入显式channel。full/declaration/signature/signature-const lowering及其跨module defs callback与type-resolution使用同一私有failure slot，signature cache hit直接返回`Ok`，捕获到dependency失败时不发布cache artifact；三类item-signature provider逐层`try_get`输入，四类normalization provider直接传播lowering/signature失败。缺失module回归扩展到`TypeNormalizationQuery`终点，signature/full normalization共享canonical type id与session append回归保持通过。compiler显式provider增至52个，legacy裸provider降至82个；下一切片应迁移value/local/semantic-use、const/layout/check等semantic链，并在所有production root显式化后删除`execute/get` adapter和公开catch boundary。

进展（2026-07-25）：I-1f 将value/local/semantic-use基础链迁入显式channel。value resolver预取full item tree/defs、graph、public surfaces与module using scope；visible extensions仍由关联值resolver按首次真实lookup惰性请求，但callback改为`QueryResult`并保存首次failure，因此普通path不会新增extension dependency，失败也不经panic。跨module defs callback复用调用级failure slot。local resolution与semantic-use table逐项`try_get`其resolution/type inputs；需要const-expression value resolution时同样使用惰性visible-extension callback并在构造semantic-use product前检查callback failure。缺失module回归扩展到`SemanticUseTableQuery`，既有value/local/type组合语义与“plain path不构建visible extensions”精度回归通过。compiler显式provider增至55个，legacy裸provider降至79个；下一切片继续迁移const/layout/module-check与extension lookup生产链。

进展（2026-07-25）：I-1g 先关闭不受executable旧callback契约牵连的const/check叶节点。标准`ConstModuleQuery`逐项显式取得active item tree、defs、value/local/semantic-use、type lowering、item signatures与source path，任一输入失败都直接返回`QueryError`；`FlowCheckQuery`同样显式传播active tree和function signature failure。缺失module回归同时覆盖两个新终点，不用unwrap、panic payload或默认空产品掩盖错误。compiler显式provider增至57个，legacy裸provider降至77个；const phase计算仍与executable/signature-const共享不可失败的program callback helper，下一切片必须迁移该helper及其真实调用者，不能复制一套“标准专用”builder形成第二条truth path。

进展（2026-07-25）：I-1h 迁移program signature到ABI check的完整闭合链。program signature module集合显式传播semantic membership与逐module eligibility failure；eligibility、module program facts和ABI facts传播signature item tree/signature product failure；program ABI aggregate逐module取得ABI facts后才发布，`AbiCheckQuery`再显式组合full defs、function/type/value signatures与program aggregate。当前query runtime尚无fallible batch入口，因此aggregate使用唯一逐项`try_get`路径，等Phase I最终把`get_many`统一为`QueryResult`时直接恢复batch，不新增panic batch wrapper或第二种聚合语义。缺失module回归覆盖module ABI facts与ABI check终点。compiler显式provider增至63个，legacy裸provider降至71个；下一切片继续处理共享const callback helper及layout/static链。

进展（2026-07-25）：I-1i 将extension discovery/signature/trait-solving facts前半链迁入显式channel。provider module eligibility与stable module集合传播provider summary/parse membership failure，discovery index逐module显式取得eligibility和summary；extension signature input显式组合defs、trait/function/type signature lowering与normalization；trait-solving persistent product在fresh路径显式取得program source identity和signature input，失败时不publish，合法cache hit仍直接返回同一typed product。trait-specific impl projection显式解析source trait defs、discovery candidates与逐moduletrait facts。缺失module回归覆盖signature input及trait-solving facts，provider membership和跨revision cache刷新保持通过。compiler显式provider增至69个，legacy裸provider降至65个；下一切片继续迁移module facts/index/visible extension产品及其不可失败lookup callback。

进展（2026-07-25）：I-1j 完成extension module facts、nominal/provider method index和visibility主链，并同步迁移其唯一外部lookup消费者value-ref edges。`SharedProgramDefsResolver`与query-backed graph/public-surface/using-scope lookup现在只记录本次算法首次`QueryError`；所有extension/value-ref消费者在发布产品或persistent artifact前统一检查failure，不把失败降级为`None`/空集合。nominal target、method name/id与visible extension/trait-impl projection逐项显式取得discovery、facts、type exposure和graph输入；`ProgramTypeAliasSignatureQuery`也显式传播module signature facts。`ExecutableValueRefItemIndex -> Item -> Edges`三层同步返回`QueryResult`，cache hit保持短路，fresh失败不publish。缺失module回归覆盖visible extensions和value-ref edges，既有dependency precision及persistent cache replacement回归通过。compiler显式provider增至78个，legacy裸provider降至56个；全query实现已有82个`execute_result`、53个裸`execute`（含测试query）。下一切片处理extension validation/trait signature index与共享program-signature/const helper。

进展（2026-07-25）：I-1k 删除program-signature aggregate的最后一条panic读取路径，并贯穿三个真实根消费者。唯一`program_signature_facts` helper现在显式取得stable module集合与逐module facts；program trait-method index、extension trait-signature index和persistent validation diagnostics均返回`QueryResult`，validation的trait-impl callback保存首次failure并在publish前退出。non-function signature aggregate同步显式化，monomorphization、backend-lowering input外层query与executable checked/provider-demand roots直接传播该failure；backend自身的用户diagnostic结果仍是query value内层`Result`，不与query infrastructure failure混合。executable extension lookup也改为调用级failure slot，session函数始终返回`(QueryResult<Output>, Session)`，因此失败后先恢复唯一fact session再向runtime返回错误。缺失module回归扩展到extension validation，mono、extension dependency与serialized executable session回归通过。compiler显式provider增至82个，legacy裸provider降至52个；全query实现已有89个`execute_result`、46个裸`execute`（含测试query）。下一切片正式迁移signature-const/standard const共享helper及layout/static/body链。

进展（2026-07-25）：I-1l 清理不受const/body callback阻塞的compiler基础query contract。`ModuleGraphQuery`及entry/path/parent/child/package-root细粒度projection、loaded module/load diagnostics、target/runtime/optimization、provider fact revision/worklist、body activation worklist、executable fact epoch与root modules共16个实现统一改为`execute_result`；derived projection逐层`try_get`，context-owned immutable input直接`Ok`，epoch逐module传播source-version failure。graph entry与activation中仅保留stable-key缺失的ICE断言，它们验证同一graph snapshot内部不变量，不识别普通输入错误。graph owner remap、provider worklist accumulation与epoch reset回归通过。compiler provider计数保持82/52，但全query实现已增至105个`execute_result`、30个裸`execute`（含3个测试query）；下一切片只剩check/body/codegen顶层与signature-const/const/layout主链。

进展（2026-07-25）：I-1m 完成signature-const与standard const共享主链的显式失败迁移，并贯穿layout/body/executable checked消费者。signature const module、array lengths、values、layout products与standard const phases统一使用`QueryResult`；program callback仍保持惰性与精确依赖，以调用级failure slot记录首次跨模块失败，算法结束或cache publish前统一检查，不把失败降级为`None`，不发布半成品。body const inputs、body check、executable layouts、signature-only checked modules、executable function/static roots和相关query均传播结构化错误；executable session在失败时仍恢复唯一owner，layout callback failure不会被后续signature fallback掩盖。missing-loaded-module回归新增signature const/layout、全部standard const phase与body check终点；compiler-query定向测试通过，显式query实现增至约120个，legacy裸实现进一步收敛。下一切片继续清理static/codegen顶层与剩余裸`execute`/`get`，随后删除临时adapter和公开catch boundary。

进展（2026-07-25）：I-1n 将普通layout与static-check根迁入显式failure channel。`LayoutsQuery`逐项传播defs、type lowering/normalization、item signatures、const array lengths与signature-layout callback失败；`StaticCheckQuery`传播resolution、semantic-use、const values、target及惰性program defs/const callbacks，算法结束前检查首次失败，不生成默认空产品。缺失module回归新增Layouts与StaticCheck终点，compiler-query全测与workspace Clippy保持通过；下一切片处理CheckedModule及codegen preparation/root链。

进展（2026-07-25）：I-1o 将`CheckedModuleQuery`及executable checked-module组装helper迁入显式channel。普通module product逐项传播body/flow、defs、resolution、type lowering/normalization、const/static/layout/ABI与semantic-use失败；executable helper同样返回`QueryResult`，仅在filtered const product缺失时惰性请求完整const query，避免引入无关依赖。缺失module回归覆盖checked-module终点，compiler-query 170项与严格workspace Clippy通过；下一切片迁移checked-module集合、program/codegen aggregate及剩余single-consumer backend query。

进展（2026-07-25）：I-1p 迁移checked-module集合、program aggregate、codegen preparation与backend input装配。`ExecutableCheckedModulesQuery`按runtime function/static key逐项`try_get`并只在全部成功后materialize module body；`CheckedModuleIdsQuery`、checked/entry program、早期与module diagnostics、codegen preparation/program外层契约均返回`QueryResult`。lowered function body、source-item/function-instance plan与body-local signature组合同步显式化；backend input不再通过`get_many`读取可失败产品，而是唯一逐项fallible路径，等最终统一batch API后再恢复并行。missing-module回归覆盖checked-program aggregate，compiler-query 170项与严格workspace Clippy通过；全query只剩5个production裸`execute`，均集中在single-consumer backend finalization链。

进展（2026-07-25）：I-1q 关闭最后五个production裸`execute`及single-consumer backend finalization failure链。`BackendItemPlanQuery`、finalization task context、externally-published module plan、module finalization与aggregate backend lowering统一实现`execute_result`；输入、optimization、owned module plan与并行worker失败逐层通过`try_get/try_get_owned`传播，不再由默认`execute`桥转成panic。唯一`with_many_owned_completion`直接升级为逐项`QueryResult<Value>` stream，没有保留panic型completion兼容入口；schedule的`wait_next/finish`和scoped consumer外层显式返回`QueryResult`，Driver只在最终artifact边界把query infrastructure error转成internal diagnostic。diagnostic plan路径重新消费已publish module plan时同样使用fallible owned读取，失败不会生成部分aggregate。runtime回归锁定owned completion中的invalid input以值返回，backend schedule/definition manifest回归覆盖live消费者；compiler production query现有0个裸`execute`，只剩3个test fixture query。下一切片删除四个公开`catch_unwind<QueryError>`与`get/try_get`双入口，再统一fallible batch API；panic只保留ICE。

进展（2026-07-25）：I-1r 删除`CompilerDatabase`四个公开check/codegen适配边界的`catch_unwind<QueryError>`与panic payload downcast。checked/entry analysis、codegen preparation/program现在只匹配底层`try_get`的显式`QueryResult`并把普通failure投影为既有internal query diagnostic；任何真正panic不再被这些边界误认为普通query失败，而是沿唯一ICE路径传播。全仓已无`downcast::<QueryError>`，下一切片统一`get/try_get`、owned与batch访问入口并删除`QueryKey::execute`默认panic桥。

进展（2026-07-25）：I-1s 补齐loader尚未迁移的13个typed product query。loaded program/diagnostics、parsed/syntax module、四类module fact、source status、module declarations、provider summary、facade facts与provider demands全部实现`execute_result`；依赖读取使用`try_get + ?`，模块aggregate逐项collect `QueryResult`。persistent frontend cache的source input与fresh item-signature helper同步fallible，任何source/syntax/parsed failure都在artifact publish前退出，不把空syntax、cache miss或旧artifact当作query failure fallback。unknown source回归锁定source-status与loaded-module终点返回同一`InvalidInput`值；loader/compiler production query现均无裸`execute`，只剩query-runtime与fixture测试实现。审计同时纠正此前“loader完整接入”的过宽表述：module graph/provider traversal仍有raw `get`与非fallible key-construction helper，下一切片必须将其错误维度与用户load diagnostic分离后再删除统一默认桥。

进展（2026-07-25）：I-1t 关闭loader module-graph与provider traversal的隐藏panic读取。新增内部`TraversalError::{Query, Diagnostic}`，所有used-path、declared-child、facade reexport、trait/provider matching递归统一返回同一窄结果；graph构建边界只把用户`Diagnostic`附加到对应module path，`QueryError`立即上浮并放弃未发布graph，不再被伪装成missing import/provider。revision-bearing parsed/declaration/public-surface/provider/facade query-key helper同步返回`QueryResult<Key>`，唯一source-version选择显式依赖`SourceStatusQuery`，production和cache publish链不再在key构造期间调用panic型`get`。测试fixture仅用required-key helper声明“source path已登记”的白盒前置条件，不恢复production兼容入口。loader graph/provider/typed-product内部raw `get`归零，剩余raw访问只位于`LoaderDatabase`公开适配与测试；下一切片统一`get/try_get`、owned与batch API并删除`QueryKey::execute`默认panic桥。

进展（2026-07-25）：I-1u 删除query provider contract的最后一个panic兼容桥。runtime、loader与compiler的全部38个测试fixture统一显式实现`execute_result -> QueryResult<Value>`，正常fixture直接返回`Ok`，fallible与cycle fixture保持结构化错误传播，专门验证真实provider panic清理/恢复的fixture仍只代表ICE路径；`QueryKey::execute`及其`QueryError -> panic_any`默认实现已物理删除，trait现在只有唯一required provider入口。全仓Rust `fn execute(`搜索归零，production与测试不再能通过实现旧方法绕过failure channel；query runtime 76、loader 74、compiler 170项及三个crate all-target check通过。Phase I现约88%；下一切片统一`get/try_get`、`get_owned/try_get_owned`和fallible batch API，随后删除访问端的panic wrapper与双轨命名。
...
进展（2026-07-25）：I-1x 迁移共享`body_check_with_filter_and_layouts_with_inputs`的全部31个raw query访问。module origins/tree/defs/type/source/normalization、local signatures、visible trait impls与target作为fallible直接输入预取；program defs、extension method/index、type normalization、八类program signature、trait-method index、item signatures、visible extensions及const/layout callback全部复用函数原有的唯一`query_failure`槽，算法需要不可失败lookup形状时只返回`None`/空临时视图继续收集，最终在构造可发布body product后统一丢弃输出并返回首次`QueryError`。没有新增resolver、fallback product或兼容入口；该主helper内raw `db.get`归零，body-executable全文件raw调用从85降至54。compiler-query 170项与crate all-target/all-features严格Clippy通过。Phase I现约90%；下一切片迁移global-initializer/const-input与value-ref closure helper，随后收口executable reachability session的18个raw入口。

进展（2026-07-26）：I-1y 关闭filtered global-initializer与body const-input链的27个panic读取。global initializer helper现在返回`QueryResult<Option<ResolvedConstExpr>>`，module defs/path/tree/public surface/using scope/source/type/signature与graph作为fallible输入，两个associated-value resolver和跨module defs callback把首次失败汇入唯一调用槽，失败时不返回或缓存部分initializer。const-input helper的program path/defs/normalization、trait impl、enum/signature、visible extension与target读取同样显式化；不可失败const算法callback遇错只返回临时`None/false`并在四阶段结束后丢弃全部结果。旧helper未保留，signature-facts分支继续走同一typed入口。该域raw `db.get`归零，body-executable全文件从54降至27；compiler-query 170项与crate all-target/all-features严格Clippy通过。Phase I现约91%；下一切片迁移value-ref closure与per-item executable roots，再处理layout/checked-module辅助链和executable reachability session。

进展（2026-07-26）：I-1z 迁移executable value-ref closure与per-item root链。closure walk逐个`try_get`cached/persistent edge query并返回`QueryResult<bool>`；edge classification显式取得跨module defs与function signatures，AST visitor因第三方Visitor callback不可失败而只保存首次`QueryError`，完整遍历结束立即上浮，失败edge index不会发布persistent artifact。reachable-item edge aggregate、filtered function expansion、local-static owner expansion及function/static per-item active tree同步返回或传播`QueryResult`；session consumer在恢复唯一fact session的同一宏边界退出，不留下部分reachability。该链raw `db.get`归零，body-executable全文件从27降至20；compiler-query 170项与crate all-target/all-features严格Clippy通过。Phase I现约92%；下一切片迁移executable layout/checked-module辅助链，并关闭executable reachability session剩余18个raw入口。

进展（2026-07-26）：I-1aa 关闭executable layout与checked-module辅助链的最后20个panic读取。reachable layout直接传播module defs/tree、type lowering/normalization与local signatures失败，跨模块aggregate signature、layout及const-array-length callback共享调用级首次failure；runtime-global判定改为`QueryResult<bool>`，不会把defs失败误判为type-only module。signature-only checked module和reachable flow check逐项显式取得输入；最终codegen root filtering返回`QueryResult<CheckedModule>`，session consumer在唯一owner恢复边界解包，失败时不发布部分module。`body_executable.rs` production raw `db.get`归零，compiler-query 170项与crate all-target/all-features严格Clippy通过。Phase I现约93%；下一切片关闭executable reachability session剩余18个raw入口。

进展（2026-07-26）：I-1ab 关闭executable reachability session的18个panic读取。worklist/epoch/module/root输入在session解构前原子`try_get`，失败时原样归还唯一session；function/type/trait signature callback共享首次failure槽，并在initial reachability、value-ref扩展、incremental extend与aggregate-root边界立即上浮，成功前不写入signature cache。runtime root、module-body demand、runtime-global分类及debug path/name helper同步返回`QueryResult`；所有中途错误均经`return_session_error!`恢复session owner，不生成部分checked-module output。`executable_reachability.rs` production raw `db.get`归零，compiler-query 170项与crate all-target/all-features严格Clippy通过。Phase I现约94%；下一切片迁移compiler剩余production单项访问与数据库外层adapter，随后删除`get/try_get`双轨入口。

进展（2026-07-26）：I-1ac 清零compiler provider/algorithm文件剩余3个raw单项访问。executable fact-state构造显式传播full defs失败，runtime-global callback以局部首次failure槽适配第三方不可失败predicate，stale/body-module选择返回`QueryResult`且session消费者统一恢复owner；backend input在进入纯组装计时区前显式取得runtime。失败的runtime-global临时`false`结果只用于完成不可中断遍历，算法边界立即丢弃整个结果，不成为fallback或第二truth source。除`query/mod.rs`外的compiler query production Rust文件现已无`db.get`，compiler-query 170项与crate all-target/all-features严格Clippy通过。Phase I现约95%；下一切片处理certificate/revision/stable-sequence外层边界，再删除runtime单项panic wrapper与双轨命名。

进展（2026-07-26）：I-1ad 关闭compiler最后4个production raw访问边界。check certificate context返回`QueryResult<Option<_>>`：合法无cache identity仍是`None`，query failure则在公开check report的唯一internal-diagnostic边界转换，不再伪装成cache miss；provider fact revision公开API直接返回`QueryResult`。stable module sequence解析显式依赖fallible module graph，program signatures、extension discovery、frontend及executable session调用者全部使用`?`传播。compiler `query/mod.rs`测试区之前与其余production query文件现均无`db.get`，170项与crate all-target/all-features严格Clippy通过。Phase I现约96%；下一切片物理删除query runtime的`get/get_owned` panic wrapper并迁移白盒测试消费者，只保留唯一fallible单项API。
...
进展（2026-07-26）：I-2a 建立唯一stable diagnostic bundle codec并迁移首个真实persistent consumer。审计确认当前只有extension validation允许非空diagnostic artifact，且`signature_cache`私有codec只支持`NAME_RESOLUTION`、单source primary label与空notes/help/related/debug，形成了第二份诊断语义。新codec由`nia-diagnostic`唯一拥有，格式只编码registered code text、summary、完整label span/source/style/message、notes、help、related与debug；severity/category从canonical code registry恢复并在写入时校验，所有span仅为当前source内offset且受source length约束，不编码`SourceId`、revision、module handle或path owner。decode限制总bytes、序列长度和基于remaining bytes的预分配，拒绝未知code、非法enum/UTF-8/span、截断与尾随bytes。extension validation删除旧窄codec并直接消费该bundle，entry magic由`NIAEV001`升级为`NIAEV002`，旧artifact不读取不迁移；完整secondary/note/help/related/debug roundtrip与corruption retirement已覆盖。Phase I仍约98%；下一切片定义跨source `ProgramDiagnostic` bundle/store owner，以stable source identity组合per-source bundle，随后让非clean check certificate能够持久化完整diagnostics而不伪造typed analysis。

进展（2026-07-26）：I-2b 建立跨source的stable `ProgramDiagnostic` store，并让check certificate持久化非clean报告。上层store以按规范化path排序的source table统一拥有source length，每条诊断只保存stable source index与唯一`nia-diagnostic` bundle；report顺序保持不变，decode必须在current exact-input source universe中重新验证path、长度和span，未知owner、source长度变化、非canonical/重复source、非法单诊断bundle、截断与尾随bytes全部拒绝，不编码`ModuleId`、revision、`SourceId`或session handle。check certificate唯一schema由`NIACC001`升级为`NIACC002`，payload新增完整`ProgramDiagnostic` store；普通错误/警告warm check现在返回current graph/current optimization与逐字段相同的diagnostics，同时`entry_checked_program`、body及semantic链保持零执行，不伪造typed analysis。verification仍fresh重算，格式合法但语义错误的clean artifact会被完整non-clean replacement取代，随后普通warm直接命中；无法绑定current source owner的诊断不发布artifact，replace验证路径会退休旧证书而非留下stale结果。program store覆盖多source、完整label/note/help/related/debug顺序与owner变化回归，compiler-query 172项和crate all-target/all-features严格Clippy通过。下一切片对Phase I第2项的session内`DiagnosticId`/bundle handle目标及第3–5项做最终acceptance审计，不能仅凭persistent store完成就把Phase I记为100%。

进展（2026-07-26）：I-audit-1 逐项重做Phase I acceptance记账并纠正此前约98%的过宽估计。第1项panic query flow已100%；第2项只有stable persistence完成，compiler各phase仍有约165处`Vec<Diagnostic>`字段/工作缓冲，尚无session owner与`DiagnosticId`；第3项仓库仍为64个crate，`query/mod.rs`已从roadmap起点约7,682行增长到约10,749行，`nia-query/src/lib.rs`约6,098行、`signature_cache.rs`约4,980行，未发生可验收的crate合并或production/test巨石拆分；第4项Driver/CLI仍由数百个手写`#[test]`内嵌source片段组成，没有case manifest、suite metadata或diagnostic snapshot runner；第5项旧query API/adapter已经删除，但integration层仍有21处显式`nia-test-support` permit/resource wrapper，开发调试环境入口也尚未分类退休。Phase I据此纠正为约55%，整份roadmap按全部显式Acceptance加权纠正为约95%；核心A–H编译器架构仍约99%，两者必须分开陈述。后续顺序固定为：先建立唯一session diagnostic store并逐域删除payload-owning query字段；并行准备data-driven suite迁移；再按dependency/owner证据决定crate合并并拆production/test巨石；最后用suite session weight替代per-case permit并完成旧入口全仓审计。完整Phase I不再承诺单日完成，但每个切片必须保持旧字段/API物理删除、严格测试和独立commit。
...
进展（2026-07-26）：I-2l 完成full `TypeResolutionQuery`及其正常/可执行checked-module报告边界迁移。query value以`ModuleTypeResolution`发布无diagnostic的`Arc<TypeResolution>`与session bundle；semantic-use、body-check、const initializer和type lowering等只读消费者统一经单向`type_resolution_semantic`边界取得语义`Arc`，不增加`Deref`、旧payload getter或兼容产品。两种full `CheckedModule`复用同一semantic handle并按既有前端顺序保存resolution/normalization bundles，codegen只在唯一report aggregation逐bundle解析，因此不会复制或遗漏类型解析错误。新增缺失类型回归与handle reuse断言，compiler-query 179项和crate all-target/all-features严格Clippy通过。Phase I约58%；下一域继续审计其余payload-owning高扇出query，随后转入crate/file重组与data-driven incremental/backend suite。

进展（2026-07-26）：I-2m 完成full `TypeLoweringQuery`及checked-module报告边界迁移。query value以`ModuleTypeLowering`发布无diagnostic的`Arc<TypeLowering>`与session bundle；semantic-use、body/executable、const evaluation、layout和program-signature等消费者统一经单向`type_lowering_semantic`边界读取语义值，只有两种full `CheckedModule`保留完整产品以同时转移semantic handle和diagnostic bundle。resolution、normalization和lowering bundles按原报告顺序保存，codegen移除对`checked.type_lowering.diagnostics`的直接读取并只在唯一report aggregation解析handle；没有`Deref`、旧getter、payload复制或双轨产品。新增generic argument mismatch回归，compiler-query 180项和crate all-target/all-features严格Clippy通过。Phase I约59%；下一域审计full item/value/local resolution等剩余高扇出query，再按依赖证据转入crate/file重组与data-driven incremental/backend suite。

进展（2026-07-26）：I-2n 完成full `ItemSignaturesQuery`诊断所有权迁移。query value以`ModuleItemSignatures`发布无diagnostic的`Arc<ItemSignatures>`与session bundle；body/executable、const evaluation、layout、normalization和backend monomorphization等所有语义消费者统一经单向`item_signatures_semantic`读取语义值。最终report aggregation仍在value/local之后取得current item-signature bundle并解包，保持既有用户可见诊断顺序，不把bundle塞入错误的前端位置；没有`Deref`、旧getter、payload复制或兼容产品。新增bodyless non-extern function回归，compiler-query 181项和crate all-target/all-features严格Clippy通过。Phase I约60%；下一域审计full value/local resolution，再继续crate/file重组与data-driven incremental/backend suite。
...
进展（2026-07-26）：I-2q 完成`FlowCheckQuery`诊断所有权迁移。query value以`ModuleFlowCheck`发布无diagnostic的semantic `Arc<FlowCheck>`和session bundle；普通checked module复用query semantic/bundle，filtered executable flow在发布semantic值前移出diagnostics并注册bundle，types-only module使用canonical empty handle。最终report在原const/static/layout/ABI之后、body之前解析`flow_diagnostics`，保持既有顺序且不再读取`checked.flow_check.diagnostics`；没有兼容getter或payload复制。新增missing-return回归与checked-module handle reuse断言，compiler-query 184项和crate all-target/all-features严格Clippy通过。Phase I仍约61%；下一步迁移static/ABI终端检查，再处理layout/const与backend aggregate diagnostics。

进展（2026-07-26）：I-2r 完成`StaticCheckQuery`与`AbiCheckQuery`诊断所有权迁移。两个query分别以`ModuleStaticCheck`和`ModuleAbiCheck`发布无diagnostic的semantic `Arc`及session bundle；普通checked module复用语义和bundle，filtered executable与types-only module均使用canonical empty handle。最终report继续按const→static→layout→ABI→flow→body顺序解析bundle，不再读取`checked.static_check.diagnostics`或`checked.abi_check.diagnostics`；没有兼容getter或payload复制。新增static initializer与extern ABI错误的分离回归及checked-module handle reuse断言，compiler-query 185项和crate all-target/all-features严格Clippy通过。Phase I仍约61%；下一步迁移layout/const与backend aggregate diagnostics，再按高扇出/报告边界继续审计。

进展（2026-07-26）：I-2s 完成full与signature `LayoutsQuery`诊断所有权迁移。两类query统一发布无diagnostic的`Arc<Layouts>`和session bundle，递归layout callback只借用semantic handle；normal checked module复用full query handle。可执行layout cache改为缓存同一semantic/bundle对，filtered与types-only module在发布`Arc`前注册本次计算的diagnostics，rooted codegen filter亦重新发布bundle；没有`Arc::make_mut`、payload clone或raw query兼容入口。最终report仍保持const→static→layout→ABI→flow→body顺序，新增recursive struct layout分离回归与full layout handle reuse断言，compiler-query 186项和crate all-target/all-features严格Clippy通过。Phase I仍约61%；下一步迁移const products，再审计backend aggregate diagnostics。

进展（2026-07-26）：I-2t 完成聚合`ConstQuery`诊断所有权迁移。正式report唯一消费的full `ConstCheck`现在以`ModuleConstCheck`发布无diagnostic semantic `Arc`和session bundle；普通checked module复用query handle，filtered executable与types-only module在发布自己的const semantic值前注册完整诊断。backend在构造其独立const input时才从bundle解包为所需owned diagnostics，未恢复query或checked-module payload。array/enum/value/typed/module phase产品保留内部算法契约，但不再通过正式报告暴露；没有compat getter或双轨产品。新增const-cycle分离回归与checked-module handle reuse断言，compiler-query 187项和crate all-target/all-features严格Clippy通过。Phase I仍约61%；下一步审计backend aggregate diagnostics。

进展（2026-07-26）：I-2u 完成program-level monomorphization与backend-lowering aggregate诊断迁移。两个正式query product均在发布边界从算法结果移出`Vec<Diagnostic>`，只缓存无diagnostic的语义`Arc`和session-owned bundle；backend module instance plan只读取monomorphization semantic，`CodegenPreparation`与`CodegenProgram`继续只暴露原始语义`Arc`。最终程序报告分别在monomorphization/backend 的既有顺序位置解包 bundle 并投影 source path，live backend finalization schedule 仍直接消费其短生命周期 raw lowering，不新增逃逸 wrapper 或第二条 finalization 路径。递归泛型深度错误回归锁定 monomorphization semantic payload 为空诊断而 bundle 完整保留；backend finalization lifecycle 回归锁定其 query semantic payload 与 bundle 的分离。没有 compatibility getter、`Deref` 或诊断 payload copy。下一步继续审计余下的 aggregate 与算法 crate 边界，优先处理正式 query/report 路径而非内部瞬时工作缓冲。

进展（2026-07-26）：I-2v 完成full `BodyCheckQuery`诊断所有权迁移。query 发布边界将既有共享`Arc<Vec<Diagnostic>>`直接转入 session bundle，因而不复制 body diagnostics；其 semantic `Arc<BodyCheck>`保留IR、facts和provider demands，但诊断字段已为空。普通`CheckedModule`复用同一semantic/bundle handle，不再重新注册或读取`BodyCheck.diagnostics`；可执行 reachability session 的增量可变工作缓冲仍保持算法内部所有权，未被错误包装成跨revision query payload。新增body return type mismatch回归锁定诊断只位于bundle，既有checked-module handle reuse回归锁定body IR/facts/provider demand与bundle同步复用。下一步继续按正式query/report边界审计剩余诊断字段，并独立推进crate/file重组与data-driven incremental/backend suite。

进展（2026-07-26）：I-2w/I-4d 补齐定义收集的正式诊断边界，并让data-driven suite覆盖同一Driver内的source revision。`ModuleDefsQuery`与`FullModuleDefsQuery`现在均发布无diagnostic的`Arc<DefCollection>`和session bundle；所有生产语义消费者统一经`module_defs_semantic`/`full_module_defs_semantic`读取定义，`CheckedModule`只在唯一report路径携带对应definition bundle。signature-only checked module也复用同一语义handle与bundle，不以空诊断或旧`DefCollection`返回面兼容。新增duplicate definition回归锁定semantic defs为空诊断、bundle完整并由checked module原样复用；既有revision、extension、const、backend与query白盒调用同步改为显式`.semantic`。data-driven check runner新增严格的`case.meta`：首个`incremental-check` case声明mode、compiler resource、entry、编辑源和编辑前/后分类；runner把fixture复制到独立临时目录，在首次check后通过`Driver::set_source`显式提交编辑，再分别比对两份稳定diagnostic snapshot，不依赖文件时间、重建Driver或就地修改fixture。`nia-compiler-query` 190项、`nia-driver` 491项测试及两个crate的严格all-target/all-feature Clippy、fmt和diff检查通过。Phase I仍约61%；剩余的crate/file重组、backend/link case metadata、suite session weight替换和临时入口退休仍未计入完成。

进展（2026-07-26）：I-4e 将首个真实backend case迁入同一data-driven compiler suite，并删除其手写资源入口。`case.meta`解析现在是唯一严格key/value入口：mode各自消费所需字段后拒绝未知或重复字段；新增`codegen`模式与`compiler` resource声明，使用同一suite session的`Driver::codegen`执行，而非把check结果冒充backend覆盖。`codegen/optimization_report`夹具稳定快照诊断分类及完整backend optimization policy/pass/change报告；原`driver_facade_formats_optimization_report`手写临时文件、独立`compiler_permit`和弱contains断言已物理删除。现有check、multi-module和incremental cases继续走同一runner，incremental仍显式提交source revision。`nia-compiler-query` 190项、`nia-driver` 490项测试及两个crate的严格all-target/all-feature Clippy、fmt和diff检查通过。Phase I仍约61%；对象写盘/link case模式及其build级session weight、其余手写integration permit、crate/file重组仍未完成。

进展（2026-07-27）：I-4f 将真实multi-module LLVM emission迁入data-driven suite并删除第二个手写permit入口。新增`emit-llvm` mode，以manifest声明entry、必含IR片段和三个query execution期望；runner调用正式`Driver::emit_llvm_ir`，验证至少一个输出module包含目标IR，同时锁定`codegen_preparation=1`、aggregate `backend_lowering=0`及两模块`backend_module_finalization=2`，不把预先生成的IR或codegen product当作LLVM覆盖。fixture以entry导入`helper.nia`，因此实际覆盖loader、跨模块finalization和LLVM emitter；旧`driver_facade_emits_llvm_ir_from_source_request`的临时文件、独立`compiler_permit`及同等断言已物理删除。`nia-compiler-query` 190项、`nia-driver` 489项测试及两个crate的严格all-target/all-feature Clippy、fmt和diff检查通过。Phase I仍约61%；native-object/link的build级session weight及case模式、其余integration permit、crate/file重组仍未完成。

进展（2026-07-27）：I-4g/I-5a 将首个真实multi-module native-object case迁入data-driven build suite，并退休对应手写permit入口。新增`emit-object` mode，manifest必须显式声明`resource=build`、entry、最小link input数和三个query execution期望；独立build-weight suite只取得一次`build_permit`并调用正式`Driver::emit_native_objects`，验证typed link inputs达到下限、每个native object payload非空，同时锁定`codegen_preparation=1`、aggregate `backend_lowering=0`及两模块`backend_module_finalization=2`。LLVM与object runner复用同一严格execution metadata解析，不引入第二套隐式约定；旧`driver_facade_emits_native_objects_from_source_request`的临时工程、独立`compiler_permit`和弱非空断言已物理删除。`nia-compiler-query` 190项、`nia-driver` 489项测试及两个crate的严格all-target/all-feature Clippy、fmt和diff检查通过。Phase I仍约61%；link/execute等其余heavy case迁移、残余integration permit与临时入口、crate边界及巨型production/test文件重组仍未完成。

进展（2026-07-27）：I-4h/I-5b 将最后一个手写link入口迁入同一data-driven build suite，并把覆盖从“link成功”提升为真实execute。新增`link-execute` mode，manifest显式声明`resource=build`、fixture source、独立virtual source path、期望exit code、backend execution计数，并以可扩展的`module.<name>=<relative-path>`字段构造严格module map；runner将case复制到唯一临时工作区，通过`Driver::set_source`注入不存在于磁盘的`.nia-build/runner/root.nia`，链接到临时输出、验证产物路径和文件存在，再实际启动并断言退出码。trait-object allocator与mapped `build_script` fixture因此同时覆盖in-memory source、module map、完整freestanding/std闭包、vtable materialization、native object、linker和process startup；当前session精确锁定`codegen_preparation=1`、aggregate `backend_lowering=0`与21个`backend_module_finalization`，没有把跨source-universe替换的session-local计数误当进程累计值。旧`in_memory_executable_preserves_trait_object_witnesses_with_module_map`的临时工程、独立`compiler_permit`和仅检查link错误的手写入口已物理删除；object与link-execute cases共享唯一`build_permit`和同一Driver。`nia-compiler-query` 190项、`nia-driver` 488项测试及两个crate的严格all-target/all-feature Clippy、fmt和diff检查通过。Phase I仍约61%；剩余permit集中在compiler suite、通用测试helper及五个Driver生命周期白盒测试，crate边界、巨型production/test文件拆分和其他临时入口仍未关闭。

进展（2026-07-27）：I-5c 关闭普通driver unit test对全局compiler permit的依赖。`common.rs`的analysis、entry、codegen、module-map与freestanding六个通用helper不再暗中获取测试资源锁，五个只验证in-memory source、revision invalidation、loader/compiler session替换、provider fixed point和incremental body类型复用的Driver白盒测试也删除各自permit；这些路径现在直接依赖生产query executor已有的进程级CPU/内存预算，不再让libtest调度与compiler内部调度形成双层限流。删除共11个unit-level入口后，`nia-driver`只剩data-driven compiler suite的一个`compiler_permit`、heavy build suite的一个`build_permit`及待迁移persistent frontend-cache integration case的一个`compiler_permit`；默认不限制`--test-threads`的488项完整libtest在55.79秒内全部通过，证明自然并发下无需恢复helper锁。`nia-compiler-query` 190项、两个crate严格all-target/all-feature Clippy、fmt与diff检查同时通过。Phase I上调至约64%；下一步把persistent diagnostic cache回归纳入显式suite resource，随后转入crate边界与巨型文件拆分。

进展（2026-07-27）：I-4i/I-5d 将最后一个driver persistent frontend-cache integration case迁入data-driven compiler suite，并完成首个巨型test harness拆分。新增`persistent-check` mode，在唯一compiler suite permit内复制fixture到临时工作区，以共享artifact cache依次运行cold、warm与verify三次独立Driver session，断言完整diagnostic snapshot完全一致、错误分类稳定且manifest指定的cache product确实落盘；`extension_validation_diagnostics` fixture保留当前两个相同`E0201`记录而不在测试迁移中改变诊断语义。旧`tests.rs`独立回归及其最后一个非-suite `compiler_permit`已物理删除，`nia-driver`现在只由compiler suite的`compiler_permit`和heavy build suite的`build_permit`声明integration资源权重。随后875行`src/tests/cases.rs`收缩为24行编排入口，manifest/path/snapshot支持、check/incremental、persistent与backend/build runner拆至四个职责单一模块，未改变fixture协议、suite顺序、Driver复用或query计数契约。默认并发`nia-driver` 487项在55.62秒通过，`nia-compiler-query` 190项、两个crate严格all-target/all-feature Clippy、fmt与diff检查通过。Phase I上调至约67%；下一步审计crate边界并继续拆分高耦合production/test文件。

进展（2026-07-27）：I-3a/I-5e 完成crate/file重组的首轮证据审计并关闭一条临时debug双轨。workspace仍有约64个crate，按代码行数与依赖扇出筛选后，`nia-span`、`nia-ir-names`等小crate不足以单独证明应合并；后续必须先建立生产consumer、稳定类型边界与循环风险矩阵，禁止仅按文件大小做破坏性合并。审计同时确认`NIA_DEBUG_EXEC_REACHABILITY`只服务人工stderr追踪，正式timing/diagnostic API无消费者依赖，因此物理删除flag、debug DTO、三处`eprintln!`和为其额外请求module path/defs/function-name的热路径。`nia-compiler-query`的首个production拆分点也已落地：146行query registry移至职责单一`query/registry.rs`，`query/mod.rs`只声明并导入模块；registry coverage回归继续验证所有声明query contract均被注册。为避免一次移动约9,600行内嵌白盒测试而形成不可审计提交，测试迁移明确留待按fixture主题分批进行。compiler-query 190项、严格all-target/all-feature Clippy、fmt与diff检查通过。Phase I上调至约68%；下一步建立crate consumer矩阵并继续拆分query context/测试主题。

进展（2026-07-27）：I-3b 将crate边界审计固化为`tools/crate_boundaries.py`及其单元测试。工具直接消费`cargo metadata --no-deps`，确定性输出每个workspace crate的非空Rust LOC、词法public item数、生产依赖、生产consumer与dev-only consumer；同一consumer同时存在normal/dev边时以生产关系为准，并支持按LOC和生产consumer数过滤。当前保守阈值只筛出47 LOC、8个public item、2个生产consumer的`nia-ir-names`，它仍只是稳定类型归属与循环风险的人工复核对象，不构成自动合并结论；高扇出的`nia-span`不会因体积小被误列。I-3c随后开始分批拆解`query/mod.rs`：90行共享query trace/symbol/test DB支持迁至`query/tests/support.rs`，117行context输入、certificate、stable module sequence和worklist数据模型迁至`query/context.rs`，仅向`query`父模块开放且不扩大crate API。主文件从10,982行降至10,800行，测试名称和数量保持不变；compiler-query 190项、严格all-target/all-feature Clippy、fmt与diff检查通过。Phase I上调至约69%；下一步验证`nia-ir-names`类型所有权，并按测试主题继续迁移fixture与context行为。

进展（2026-07-27）：I-3d 完成首个低扇出crate的稳定边界复核，并继续分批迁移compiler query白盒fixture。`nia-ir-names`的`LocalName`/`GeneratedLocalName`同时由typed body IR与lowered function IR持有，两侧均直接依赖并重导出给各自下游；合入任一peer会制造反向层级依赖，合入`nia-symbol`则会把IR临时量/生成名语义倒灌基础符号层，因此确认它是必要的共享value-type边界，不能为减少crate数量而合并。测试重组方面，188行module graph/source revision/loaded-module构造迁至`query/tests/fixture.rs`，365行测试专用loader query、fact DTO与`LoaderFactProvider`适配器迁至`query/tests/loader_facts.rs`；父模块分别只保留现有fixture调用和业务测试数据库包装。`query/mod.rs`进一步从10,800行降至10,250行，compiler-query连续两轮均为190项全绿，严格all-target/all-feature Clippy、fmt与diff检查通过。Phase I上调至约70%；下一步扩大consumer matrix复核范围，并继续按frontend invalidation、persistent cache与backend主题迁移测试用例。

进展（2026-07-27）：I-3e 将边界复核阈值扩大到500非空Rust LOC与最多4个生产consumer。新增进入集合的`nia-ice`由CLI、driver、LLVM wrapper与LLVM codegen共同消费，负责panic payload到internal diagnostic的单向隔离；`nia-sema`由body/const check与const eval共同消费，集中无owner状态的arity、array length及field-set纯检查算法；`nia-test-support`没有生产consumer，只为CLI/driver两个dev consumer提供query permit与临时资源协调。连同已确认的`nia-ir-names`，四者均有跨peer复用或test-only基础设施职责，当前阈值内没有证据充分的合并对象，禁止为了减少64个workspace成员而破坏依赖方向。I-3f继续迁移白盒基础设施：139行compiler update/check/codegen测试数据库与loader materialization进入`query/tests/database.rs`，177行普通/持久frontend cache query DB、source fingerprint loader及stable identity映射进入`query/tests/frontend_cache.rs`；测试包装器通过显式import与production `CompilerDatabase`隔离，避免glob名称双轨。`query/mod.rs`从10,250行降至9,941行，首次低于一万行；两批迁移各自通过compiler-query 190项、严格all-target/all-feature Clippy、fmt与diff检查。Phase I上调至约71%；下一步扩大到中等规模单consumer crate，并按frontend invalidation/persistent cache测试用例主题迁移实际case而不仅是support。

进展（2026-07-27）：I-3g 将边界复核扩大到1,200非空Rust LOC且最多1个生产consumer。`nia-abi-check`、`nia-flow-check`与`nia-static-check`均为compiler-query独占调度但由backend测试复用或拥有独立terminal-check语义的phase crate；`nia-build`则是CLI唯一生产consumer背后的driver/loader/source/timing构建编排层，合入CLI会倒置库与入口边界，因此本轮仍无可安全合并对象。测试文件拆分首次从support进入实际case：10个module membership、stable graph/source identity、tracked loader field与timing-only update回归整体迁至`query/tests/frontend_invalidation.rs`，只增加主题路径，不改变函数名、fixture、断言或数量。`query/mod.rs`从9,941行降至9,613行；compiler-query 190项、严格all-target/all-feature Clippy、fmt与diff检查通过。Phase I上调至约72%；下一步迁移provider worklist与persistent frontend cache实际case，并继续复核更大单consumer crate。

进展（2026-07-27）：I-3h 将单生产consumer复核阈值扩大到2,000非空Rust LOC。新增`nia-monomorphize`与`nia-public-surface`均由compiler-query统一调度但拥有独立算法产品和backend/test复用边界，`nia-cli`则是无生产consumer的最终入口；连同此前terminal-check/build对象，本轮仍无证据充分的合并候选。测试重组将15个provider graph growth、fact revision/worklist/reset、shared executable session、fact epoch与content-identical reuse实际回归整体迁至`query/tests/provider_incremental.rs`，不改变函数、fixture、断言或总数。`query/mod.rs`从9,613行降至8,943行；compiler-query 190项、严格all-target/all-feature Clippy、fmt与diff检查通过。Phase I上调至约73%；下一步迁移persistent certificate/value-ref cache cases，并继续审计更大单consumer crate的owner边界。
...
进展（2026-07-27）：I-3aa 将visible extension对signature type normalization、target-scoped nominal provider modules、type exposure与精确provider module facts依赖的综合回归迁至`query/tests/extension_nominal_queries.rs`；所有正反向依赖断言与测试总数保持不变。`query/mod.rs`降至4,782行，compiler-query 190项、严格all-target/all-feature Clippy、fmt与diff检查通过。Phase I保持约75%；下一步按body/value/const resolution主题继续拆分剩余实际cases。
...
进展（2026-07-28）：I-3bu 将默认const mutation、body/extension mutation及optimization level的 lowering 配置包装器迁至`nia-backend-lower/src/tests/lowering_wrappers.rs`。所有包装器继续委托父harness中唯一的body-check mutation核心 pipeline，且仅在tests私有范围向主题模块暴露；`tests.rs`从401行降至329行。backend-lower 100项、严格all-target/all-feature Clippy、fmt与diff检查通过。Phase I保持约75%；下一步迁移唯一的核心 lowering pipeline，使父harness成为纯模块挂载与共享导入入口。
...
进展（2026-07-28）：I-3ch 将key-order、共享cache handle、owned move、typed completion stream、失败值、父依赖和session budget共7条batch访问契约迁至`nia-query/src/tests/batch_access.rs`。completion-order fixture从三个phase收敛为两个逆序phase，仍验证completion position且不再要求三个lane同时前进；主题测试在并发过滤运行下稳定通过。Phase I保持约75%；下一步迁移session task基础调度契约。

进展（2026-07-28）：I-3ci 将session task的不可Clone输出顺序、共享executor budget和bounded worker lane共3条回归迁至`nia-query/src/tests/session_tasks.rs`。priority queue、公平性与跨session预算继续留给独立executor主题，避免新文件再次混合所有调度职责；3项主题测试通过。Phase I保持约75%；下一步迁移query错误与panic恢复边界。
