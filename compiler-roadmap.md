...
| D 统一依赖图 | 约 94% | loader/provider update 已收敛为 append-only revision events 与 revision-keyed graph query，compiler update 已由 cache-owned provider/body worklist 与 executable epoch 驱动；loader/compiler typed DB 共享同一 `QuerySession`，compiler graph/membership/diagnostics 及稳定 module path/source/provider-summary roots 已直接依赖 loader facts，Driver 的 `LoadedProgram → CompileRequest` 与 pending program/revision DTO 已删除。item-tree/origin/parse field snapshot、provider lifecycle 和 reachability scheduler 尚未完全进入统一图。 |
...
综合判断：**整份路线图按剩余工程复杂度加权约完成 55%，合理区间为 53%–57%；A–E 的 P0 基础约完成 78%–82%。** 已完成的是最先阻塞后续工作的 type identity、query storage、loader-owned provider revision graph、typed compiler worklist、跨 typed-DB QuerySession recorder、首批生产 loader→compiler dependency edge、通用 derived red-green 内核路径及稳定 compiler/loader module-source facts；module-field fact migration、executor、item/CGU 粒度和持久增量仍是独立工程，不能按提交数量线性外推后半程。

ID/arena 专项已关闭，当前临界路径继续推进 Phase D：driver 重复摘要和 compiler fact-session update mutation 已删除，provider/body delta 与 executable reset 都由 typed query input 表达；loader graph 是 provider revision-keyed query product，loader/compiler typed DB 共享同一 QuerySession dependency arena，graph/membership/diagnostics 与 stable module path/source/provider-summary production roots 已真实调用 loader query并记录跨 DB edge，Driver 不再物化或传递 `LoadedProgram`，symbol/target/runtime 也只从 loader fact owner 读取。下一步先在 loader 建立 item-tree/origin/parse 等 field-level stable facts，再迁移 compiler 对应 roots并删除 `CompilerInputs::new` 的 aggregate loaded snapshot；不能让这些无 fingerprint roots直接依赖 `LoadedModuleQuery`，否则任意 source change 会扩大清图。之后再收敛 provider lifecycle/reachability worklist。Phase A 剩余 CI/trend storage 在托管环境和基线存储策略明确前不抢占该路径。
