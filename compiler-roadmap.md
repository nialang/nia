...
| D 统一依赖图 | 约 87% | loader/provider update 已收敛为 append-only revision events 与 revision-keyed graph query，compiler update 已由 cache-owned provider/body worklist 与 executable epoch 驱动；通用 red-green validation 已覆盖 module/source/membership/relation/eligibility 主链。loader 的 graph state、source version map 与 update cache mutation均已删除；loader/compiler 单一 session DB、driver provider lifecycle 和 reachability scheduler 尚未完成。 |
...
综合判断：**整份路线图按剩余工程复杂度加权约完成 53%，合理区间为 51%–55%；A–E 的 P0 基础约完成 75%–80%。** 已完成的是最先阻塞后续工作的 type identity、query storage、loader-owned provider revision graph、typed compiler worklist、通用 derived red-green 内核路径及稳定 compiler/loader module-source facts；跨数据库统一 fact graph、executor、item/CGU 粒度和持久增量仍是独立的大型工程，不能按提交数量线性外推后半程。

ID/arena 专项已关闭，当前临界路径继续推进 Phase D：driver 重复协议和 compiler fact-session update mutation 已删除，provider/body delta 与 executable reset 都由 typed query input 表达；stable derived slot 已锁定 validation/invalidation 并发状态、连续 revision 与 `get_many` 依赖记录，graph relation、loaded membership、source status/version 与 public/using relation query 都进入统一 validation 路径。loader graph 现在是 provider revision-keyed query product，`LoaderGraphState` 已删除。下一步按 crate dependency、context ownership 和 update transaction 三层审计 loader/compiler 单一 session DB 的原子迁移边界，再把 driver provider lifecycle 与 reachability worklist 接入同一 recorder；不能用跨 DB 摘要或 facade 假装统一。Phase A 剩余 CI/trend storage 在托管环境和基线存储策略明确前不抢占该路径。
