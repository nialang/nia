...
| C query value/storage | 约 70% | cache-owned `get/try_get/get_many` 与无 `Value: Clone` 已建立；loader、compiler semantic/body/check/fact batch/const facts、executable function lowering 与 codegen 大产品已共享，compiler-query 的 legacy `query_many` 已清零，剩余 owned `query`、runtime adapter 与 registry 尚未完成。 |
