...
| C 后 ID/arena 专项 | 约 58% | ID-0 query dep-node arena 与 ID-1 source/syntax identity 已完成；ID-2 已建立 stable/local module 映射、删除 full-node query，并让 loader/compiler/checked/codegen 共享 immutable graph snapshot；production 已收敛到显式 allocator，下一步迁移测试 fixture 与公共 API。 |
