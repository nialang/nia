...
| G CGU / 异步 codegen / work products | 约 68% | deterministic typed source partition、跨session stable CGU key、versioned content fingerprint、owned readonly whole-program context、统一session codegen queue、per-unit LLVM ownership/memory backpressure、持久native object work-product cache与typed incremental link inputs已完成；source ordinal仍未细分，frontend/LLVM尚未overlap，也没有link-result reuse。 |
