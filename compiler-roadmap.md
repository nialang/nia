...
| C 后 ID/arena 专项 | 约 56% | ID-0 query dep-node arena 与 ID-1 source/syntax identity 已完成；ID-2 已建立 stable/local module 映射、删除 full-node query，并让 loader/compiler/checked/codegen 共享 immutable graph snapshot；下一步收敛 allocator 外 `ModuleId` 构造。 |
...
最近的临界路径已进入 ID-2 module identity 的 allocator 收口：stable module key/local id 映射和 immutable graph snapshot 已建立，下一步先把 production 中 allocator 外的 `ModuleId` 数字构造归零，再按测试 fixture 与公共 API 分层私有化 tuple 字段并增加 foreign-owner/stale-handle 规则；Phase A 剩余 CI/trend storage 在托管环境和基线存储策略明确前不抢占该路径。
