...
| I 错误、诊断与工程重组 | 约 96% | query runtime 已建立唯一显式 provider failure contract；loader/compiler query、single-consumer backend finalization及全部runtime/test fixture均只实现`execute_result`，`QueryKey::execute` panic桥已物理删除；shared/owned batch访问统一返回`QueryResult`，compiler全部production访问端均已显式传播或在唯一公开diagnostic边界转换失败。其余`get/try_get`双轨runtime API、diagnostic store、data-driven integration harness、外层session weight替代方案和crate/巨型文件重组仍待推进。 |
