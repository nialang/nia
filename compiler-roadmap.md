...
| G CGU / 异步 codegen / work products | 约 88% | deterministic typed source partition、跨session stable CGU key、versioned CGU/link-result component fingerprint、owned readonly whole-program context、统一session codegen queue、per-unit LLVM ownership/memory backpressure、持久native object/link-result cache、typed incremental link inputs及两层精确invalidation reason已完成；source ordinal仍未细分，frontend/LLVM尚未overlap。 |
