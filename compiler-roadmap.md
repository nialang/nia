...
| F IR ownership / item 粒度 | 约 80% | checked body、static init与lowered Function IR均已成为item-owned product；backend只借用query-owned function/static IR，无consumer的过渡store/ID、batch API与`BodyIr` backend入口已删除。monomorphization/backend aggregate已有production query owner，per-module frontend source-item与function-instance plan也已成为tracked semantic product；跨模块动态closure已收敛为deterministic iteration snapshot，function/global instance的post-substitution payload与引用发现也已成为closed delta并拒绝缺失owner，但vtable query plan、per-item/per-CGU materialization、owned extraction、及时释放和peak-live-bytes验证尚未建立。 |
