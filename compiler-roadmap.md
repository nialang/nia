...
| G CGU / 异步 codegen / work products | 约 35% | deterministic typed source partition、owned readonly whole-program context、统一session codegen queue、per-unit LLVM ownership与memory backpressure已完成；source ordinal仍未细分，frontend/LLVM尚未overlap，也没有stable CGU fingerprint、object work-product cache或incremental link input。 |
