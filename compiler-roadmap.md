...
进展（2026-07-24）：G-6e5j 将正式CGU计划冻结点前移到backend item closure已经收敛、module finalization尚未启动的唯一时刻。`CodegenPartitionPlan::from_modules`现在可从借用module序列构造，`BackendItemPlan::codegen_partition_plan`在module plan被逐项move进single-consumer query slot之前计算stable key顺序、definition membership、source bucket和唯一vtable约束；plan只保存typed unit identity、module ordinal与item position，不复制Backend IR。唯一`BackendFinalizationSchedule`持有该plan并在首个`wait_next`之前公开借用，aggregate `finish`重新从canonical finalized modules计算并强制相等，任何optimization/finalization意外改变definition集合都会ICE，不建立pre-plan/post-plan双轨。两模块scope回归在store尚无ready payload时已经验证两个source partition owner，并继续证明首个completion只发布对应module；backend-ir 18项、backend-lower 100项、compiler-query 149项通过。LLVM仍未由readiness事件启动，故完成度不虚增；下一切片的coordinator现在可以直接遍历冻结plan，以`owner ready → membership Ready/Pending → exact owner retry`为唯一launch协议。
