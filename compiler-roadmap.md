...
| G CGU / 异步 codegen / work products | 约 99% | deterministic bounded source bucket、跨session stable CGU key、versioned CGU/link-result component fingerprint、owned readonly whole-program context、资源有界LLVM worker lanes、持久native object/link-result cache、typed incremental link inputs及两层精确invalidation reason已完成；ready partition现以bounded priority task接入同一persistent executor，可与尚未完成的frontend finalization真实overlap。剩余工作是代表性workload的最终CPU/RSS acceptance与telemetry收口。 |
