...
| F IR ownership / item 粒度 | 约 40% | interner 已从body/backend产品移除；checked body payload由module aggregate与semantic-value item query共享，lowered Function IR已成为per-function query product，单body变化可green复用无关lowered body；无consumer的过渡store/ID与batch API已删除。checked-body item producer、owned extraction、及时释放、per-CGU lowering和peak-live-bytes验证尚未建立。 |
