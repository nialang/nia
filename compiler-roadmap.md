...
| D 统一依赖图 | 约 97% | loader/provider update 已收敛为 append-only revision events 与 revision-keyed graph query，compiler update 已由 cache-owned provider/body worklist 与 executable epoch 驱动；loader/compiler typed DB 共享同一 `QuerySession`，compiler graph/membership/diagnostics、全部 module field roots及 public/using derived facts 已进入统一图，Driver 的 `LoadedProgram → CompileRequest` 与 pending program/revision DTO 已删除。eager executable index、aggregate loaded snapshot、standalone update diff、provider lifecycle 和 reachability scheduler 尚未完全进入统一图。 |
...
综合判断：**整份路线图按剩余工程复杂度加权约完成 57%，合理区间为 55%–59%；A–E 的 P0 基础约完成 80%–84%。** 已完成的是最先阻塞后续工作的 type identity、query storage、loader-owned provider revision graph、typed compiler worklist、跨 typed-DB QuerySession recorder、production loader→compiler module field edges、通用 fingerprint/value-equality red-green 内核路径及 query-derived public/using facts；aggregate loaded snapshot删除、executor、item/CGU 粒度和持久增量仍是独立工程，不能按提交数量线性外推后半程。

ID/arena 专项已关闭，当前临界路径继续推进 Phase D：driver 重复摘要和 compiler fact-session update mutation 已删除，provider/body delta 与 executable reset 都由 typed query input 表达；loader graph 是 provider revision-keyed query product，loader/compiler typed DB 共享同一 QuerySession dependency arena，graph/membership/diagnostics、全部 module field roots及 public/using aggregate/module/name projections均已成为 tracked query facts，Driver 不再物化或传递 `LoadedProgram`，symbol/target/runtime 也只从 loader fact owner 读取。下一步把 `CompilerInputs::new` 中 eager executable value-ref index迁成 query facts，并继续删除 aggregate loaded module snapshot与对应 standalone diff；随后收敛 provider lifecycle/reachability worklist。Phase A 剩余 CI/trend storage 在托管环境和基线存储策略明确前不抢占该路径。
