...
| D 统一依赖图 | 约 22% | driver 的 lossy graph 摘要与重复 provider 编排已删除，provider keys 全程 canonical/stable；loader 已有正式 `ProviderDemandsQuery → ModuleGraphQuery` 依赖边，不再直接 blanket invalidate graph，但 graph state 仍复制 applied-demand set，且尚未与 compiler/reachability 共享 revisioned fact graph。 |
...
ID/arena 专项已关闭，最近的临界路径正式进入 Phase D 统一模块/provider 依赖图：driver 重复协议已删除，provider dependency keys 已 canonical/stable，loader graph 也开始通过正式 provider fact query 依赖传递失效；下一步删除 graph state 的 applied-demand 重复集合，改为 fact revision/delta worklist；Phase A 剩余 CI/trend storage 在托管环境和基线存储策略明确前不抢占该路径。
