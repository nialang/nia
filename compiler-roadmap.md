...
| I 错误、诊断与工程重组 | 约 15% | query runtime 已建立显式 provider failure channel，cycle、invalid input 与 speculative dependency cleanup 不再要求 runtime downcast panic payload；loader 的 source/module-graph/loaded-module/public-surface invalid-input 边界已迁移。裸 `execute`/`get` 迁移 adapter、compiler provider graph、diagnostic store、data-driven integration harness、外层 session weight替代方案和 crate/巨型文件重组仍待推进。 |
