...
| I 错误、诊断与工程重组 | 约 91% | query runtime 已建立唯一显式 provider failure contract；loader/compiler query、single-consumer backend finalization及全部runtime/test fixture均只实现`execute_result`，`QueryKey::execute` panic桥已物理删除；shared/owned batch访问统一返回`QueryResult`，body resolution、body-check program callback与filtered const/global-initializer链也已显式传播失败。其余单项`get/try_get`与外层adapter、diagnostic store、data-driven integration harness、外层session weight替代方案和crate/巨型文件重组仍待推进。 |
