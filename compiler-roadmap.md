...
| C query value/storage | 约 84% | cache-owned `get/try_get/get_many` 与无 `Value: Clone` 已建立；loader、compiler semantic/body/check/definition/signature/fact batch/const facts/item-tree snapshots、visible extension/public surface/using scope、executable function lowering 与 codegen 大产品已共享，owned `query_many` runtime adapter 已删除，剩余 extension index 与 owned `query/try_query` 边界、registry 尚未完成。 |
