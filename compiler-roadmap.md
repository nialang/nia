...
| I 错误、诊断与工程重组 | 约 68% | query runtime 已建立显式 provider failure channel；loader、frontend type/value、extension全链、program-signature及其mono/backend/executable消费者、signature/standard const、layout/static、body 与 executable checked 根链，以及module-graph/config/worklist/epoch基础query均已接入显式失败通道。裸 `execute`/`get` 迁移 adapter、其余 checked/codegen根链、diagnostic store、data-driven integration harness、外层 session weight替代方案和 crate/巨型文件重组仍待推进。 |
