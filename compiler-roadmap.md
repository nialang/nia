...
| C 后 ID/arena 专项 | 约 66% | ID-0 query dep-node arena 与 ID-1 source/syntax identity 已完成；ID-2 已建立 stable/local module 映射、删除 full-node query，并让 loader/compiler/checked/codegen 共享 immutable graph snapshot；production 已收敛到显式 allocator，单模块 fixture 迁移已覆盖前端、checked 与 backend pipeline。 |
