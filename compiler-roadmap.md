...
| C query value/storage | 约 20% | `QueryKey::Value` 的通用 `Clone` 约束已删除，cache-owned `get/try_get` 已建立，旧 shared 入口与 loader owned 调用清零；compiler resolution product 已共享到 `CheckedModule`，其余 owned `query/query_many`、aggregate product 与 declarative registry 尚未完成。 |
