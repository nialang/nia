...
transfer release that block before retiring the list state. Growth and shrink
also resolve the old allocation from `storageBlock`; only an ownerless,
zero-capacity list may synthesize the canonical empty Block. `deinit` attempts
...
Named-struct constant construction is fallible at the wrapper boundary. It
checks the supplied value count against the physical struct field count and
requires every value's LLVM type to match its corresponding field before
...
LLVM function-signature construction is fallible: parameter counts are checked
...
Build construction is also an explicit transaction. `Build::init` returns a
`BuildInitAttempt` containing the partial graph and primary initialization
...
Owning step payloads enter the graph through a kind-correct pending `Step`.
...
The freestanding Linux x86_64 and i686 entry points derive `argv` and `envp` from the
...
Linux page mapping validates the complete mapped-range end before exposing an
aligned user pointer. When extra pages are needed for over-alignment, the
