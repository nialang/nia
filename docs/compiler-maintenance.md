...
- Function builtin integer values must be range-checked against their result
  primitive before LLVM constant construction. This includes signedness,
...
- Enum variant values must fit the enum's declared backing primitive before
  LLVM tag construction. Backend metadata must not rely on LLVM's integer
  truncation to recover an out-of-range explicit discriminant.
