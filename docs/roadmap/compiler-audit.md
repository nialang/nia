...
- [x] Batch 721 records standard build-protocol integration coverage. The
      `nia-cli` tests for dynamic argv/import limits, process and filesystem
      error context, configuration-only action suppression, partial-input
      allocation rollback, and host/artifact target separation all pass (5
...
- [x] Batch 732 records cross-cutting driver, linker, loader, and query owner
      evidence. Strict Rustdoc is clean for `nia-driver`, `nia-linker`,
      `nia-loader-query`, and `nia-query`; their complete suites pass 865 tests
      total (655, 31, 96, and 83 respectively) with strict all-target Clippy.
      Coverage includes cache and archive corruption/replacement, target and
      linker identity, module/provider graph invalidation, persistent product
      verification, query lifecycle/cycle recovery, shared execution budgets,
...
- [x] Batch 734 records Phase E parser, syntax, node-identity, AST, and local
      resolution evidence. Strict Rustdoc is clean for `nia-parser`,
      `nia-syntax`, `nia-node-id`, `nia-ast`, and `nia-local-resolve`; the
      crates with owner tests pass 170 tests total with strict all-target
      Clippy, while the AST crate remains a documented schema owner without
      unit tests. Coverage includes malformed-input progress, speculative
      origin rollback, lossless delimiter and Unicode recovery, partial
      reparsing, cross-revision/generation identity, lexical capture rules, and
      active-item-tree local resolution. Phase-wide structural review and final
      clean/incremental, resource, and external-target evidence remain open.
- [x] Batch 735 closes the strict Rustdoc backlog for `nia-ty`. The canonical
      type-store API, every `TyKind` form and supporting const/type records, and
      recursive `substitute_ty` behavior now have owner-facing documentation;
      strict `cargo rustdoc -p nia-ty --lib -- -D missing-docs`, all 16 owner
...
- [x] Batch 736 removes seven `missing_docs` suppressions from
      ...
      resource, and external-target evidence remain open.
- [x] Batch 737 removes the codec and handoff `missing_docs` suppressions from
      `nia-build`. Every bounded plan encoding and durable publication error
      now documents its variant and payload fields. Strict Rustdoc, 216 owner
      tests (one expected cross-process test ignored), strict all-target Clippy,
      crate-local formatting, and diff checks pass; the workspace formatting
...
- [x] Batch 740 removes both coordinator documentation suppressions. External
      ...
      error documentation and final acceptance evidence remain open.
- [x] Batch 741 removes the final repository `missing_docs` suppression by
      documenting every top-level `BuildError` bootstrap, runner, handoff,
      execution, and cleanup variant and payload. The complete `nia-build`
      owner suite passes 216 tests with one expected cross-process worker
...
- [x] Batch 743 documents the representative-build baseline schema and
      collection pipeline. Action outcomes, process/stage measurements,
      artifact equivalence, acceptance checks, distributions, state summaries,
      repetition samples, and schema-v5 report fields now describe their
      evidence ownership; workload parsing, corruption, and summarization
...
- [x] Batch 745 documents compiler performance baseline options, deterministic
      large-codegen source generation, workload command construction, and report
      collection ownership. Its 6 compiler-baseline owner tests, strict Clippy,
...
- [x] Batch 761 closes the intrinsic trait argument-shape boundary. Builtin
      trait resolution now rejects any non-empty const-argument vector before
      dispatching intrinsic implementations; malformed goals can no longer
      obtain valid operator/marker results or associated constants by injecting
      unsupported arguments. `nia-trait-solve` (25), `nia-body-check` (279),
...
- [x] Batch 765 closes two direct lexer boundary gaps: lossless token streams
      ...
      open.
- [x] Batch 766 adds diagnostic-rendering coverage for UTF-8 spans that begin
      inside a code point and for CRLF line mapping. Rendering clamps to a
      valid character boundary while preserving the correct line, column, and
      caret width; `nia-diagnostic` (19 tests) and the `nia-driver` consumer
      suite (655 tests) pass with strict Rustdoc, Clippy, formatting, and diff
      checks. Broader Phase E diagnostic-identity acceptance remains open.
- [x] Batch 767 adds an AST-walker regression for static declarations nested
      in expression blocks, closures, `if` branches, and `match` arms. The
      traversal now has direct source-order evidence across every nested owner;
...
- [x] Batch 795 records the post-snapshot full workspace regression gate.
      ...
      tracked separately.
- [x] Batch 796 records an independent low-memory runtime rerun. Under a
      temporary Linux cgroup scope with `MemoryMax=3G` and no swap,
      `NIA_TEST_RESOURCE_TRACE=1 cargo test -p nia-cli --test
      emit_exe_intrinsics --all-features` passes 8/8. The resource pools derive
      compiler/runtime capacity 6 from the 3 GiB limit; managed compiler
...
- [x] Batch 805 completes the LLVM backend's ILP32 test matrix. Target-width
      assertions now derive native LLVM integer types for pointer-sized
      intrinsics, vector indexes, range values, promoted constants, trait and
      structural-extension calls, and signed comparisons; vtable overflow
...
- [x] Batch 874 reruns the complete workspace acceptance matrix after the
      body-check slice inference repair. `cargo test --workspace
